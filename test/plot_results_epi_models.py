...
for assay_type in ["HiChIP", "HiC"]:
    for fdr in ["1", "01", "001"]:
        df1 = pd.read_csv(
            data_path
            + "/results/csv/cage_prediction/epi_models/R_NLL_epi_models_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
...
g = sns.catplot(
    ...
    data=df_sub,
    kind="box",
    palette={"Epi-GraphReg": "orange", "Epi-CNN": "deepskyblue"},
    height=4,
    aspect=1,
    sharey=True,
)
    ...
    for j in range(n_col):
        ...
        if g.col_names[j] in ["K562", "GM12878"]:
            ...
            add_stat_annotation(
                ...
                x="FDR",
                y="R",
                box_pairs=[(0.001, 0.01), (0.001, 0.1), (0.01, 0.1)],
                test="Wilcoxon",
                text_format="star",
                loc="inside",
                verbose=0,
...
df = pd.read_csv(
    data_path
    + "/results/csv/cage_prediction/epi_models/cage_predictions_epi_models_"
    + cell_line
    + "_"
    + assay_type
    + "_FDR_"
...
df["pred_cage_epi_graphreg_logfc"] = np.log2(
    (df_1["pred_cage_epi_graphreg"] + 1) / (df_2["pred_cage_epi_graphreg"] + 1)
)
...
for i in range(50):
    ...
    mse_graphreg_ex = np.mean(
        (df_sub_expressed["pred_cage_epi_graphreg_logfc"].values - df_sub_expressed["true_cage_logfc"].values) ** 2
    )
    mse_graphreg_ex_int = np.mean(
        (df_sub_interacting["pred_cage_epi_graphreg_logfc"].values - df_sub_interacting["true_cage_logfc"].values) ** 2
    )

    mse_cnn_all = np.mean((df_sub["pred_cage_epi_cnn_logfc"].values - df_sub["true_cage_logfc"].values) ** 2)
    mse_cnn_ex = np.mean(
        (df_sub_expressed["pred_cage_epi_cnn_logfc"].values - df_sub_expressed["true_cage_logfc"].values) ** 2
    )
    mse_cnn_ex_int = np.mean(
        (df_sub_interacting["pred_cage_epi_cnn_logfc"].values - df_sub_interacting["true_cage_logfc"].values) ** 2
    )
    ...
    r_graphreg_ex = np.corrcoef(
        ...
        df_sub_expressed["pred_cage_epi_graphreg_logfc"],
    )[0, 1]
    r_graphreg_ex_int = np.corrcoef(
        df_sub_interacting["true_cage_logfc"].values,
        df_sub_interacting["pred_cage_epi_graphreg_logfc"],
    )[0, 1]

    r_cnn_all = np.corrcoef(df_sub["true_cage_logfc"].values, df_sub["pred_cage_epi_cnn_logfc"])[0, 1]
    r_cnn_ex = np.corrcoef(
        df_sub_expressed["true_cage_logfc"].values,
        df_sub_expressed["pred_cage_epi_cnn_logfc"],
    )[0, 1]
    r_cnn_ex_int = np.corrcoef(
        df_sub_interacting["true_cage_logfc"].values,
        df_sub_interacting["pred_cage_epi_cnn_logfc"],
    )[0, 1]
...
plt.tight_layout()
plt.savefig(
    "../figs/Epi-models/final/boxplot_R_logfc_"
    + cell_line_1
    + "_"
    + assay_type_1
    + "_FDR_"
...
plt.tight_layout()
plt.savefig(
    "../figs/Epi-models/final/boxplot_MSE_logfc_"
    + cell_line_1
    + "_"
    + assay_type_1
    + "_FDR_"
...
for cell_line in ["K562", "GM12878", "hESC", "mESC"]:
    ...
    for assay_type in assay_type_list:
        for fdr in ["1", "01", "001"]:
            ...
            r_graphreg_ex = np.corrcoef(
                ...
                df_expressed["pred_cage_epi_graphreg_log2"],
            )[0, 1]
            r_graphreg_ex_int = np.corrcoef(
                df_expressed_interacting["true_cage_log2"].values,
                df_expressed_interacting["pred_cage_epi_graphreg_log2"],
            )[0, 1]

            r_cnn_all = np.corrcoef(df["true_cage_log2"].values, df["pred_cage_epi_cnn_log2"])[0, 1]
            r_cnn_ex = np.corrcoef(
                df_expressed["true_cage_log2"].values,
                df_expressed["pred_cage_epi_cnn_log2"],
            )[0, 1]
            r_cnn_ex_int = np.corrcoef(
                df_expressed_interacting["true_cage_log2"].values,
                df_expressed_interacting["pred_cage_epi_cnn_log2"],
            )[0, 1]
            ...
            norm = plt.Normalize(
                df_expressed["true_cage_log2"].min(),
                df_expressed["true_cage_log2"].max(),
            )
            ...
            plt.tight_layout()
            plt.savefig(
                "../figs/Epi-models/final/scatterplot/scatterplot_delta_nll_hue_cage_"
                + cell_line
                + "_"
                + assay_type
                + "_FDR_"
            ...
            norm = plt.Normalize(
                df_expressed["n_contact_log2"].min(),
                df_expressed["n_contact_log2"].max(),
            )
            sm = plt.cm.ScalarMappable(cmap="viridis_r", norm=norm)
            sm.set_array([])
            ax.get_legend().remove()
            ax.figure.colorbar(sm, label="log2 (N + 1)")
            ax.set_title(f"Epi-CNN | {cell_line} | R = {r_cnn_ex:5.3f} | NLL = {nll_cnn_ex_mean:6.2f}")
            ax.set_xlabel("log2 (true + 1)")
            ax.set_ylabel("log2 (pred + 1)")
            plt.tight_layout()
            plt.savefig(
                "../figs/Epi-models/final/scatterplot/scatterplot_cage_vs_pred_epi_cnn_"
                + cell_line
                + "_"
                + assay_type
                + "_FDR_"
            ...
            ax.get_legend().remove()
            ax.figure.colorbar(sm, label="log2 (N + 1)")
            ax.set_title(f"Epi-GraphReg | {cell_line} | R = {r_graphreg_ex:5.3f} | NLL = {nll_graphreg_ex_mean:6.2f}")
            ax.set_xlabel("log2 (true + 1)")
            ax.set_ylabel("log2 (pred + 1)")
            plt.tight_layout()
            plt.savefig(
                "../figs/Epi-models/final/scatterplot/scatterplot_cage_vs_pred_epi_graphreg_"
                + cell_line
                + "_"
                + assay_type
                + "_FDR_"
    ...
    g = sns.catplot(
        ...
        row="3D Data",
        col="Genes",
        data=df_R_NLL_remove_cnn,
        kind="bar",
        palette={0.1: "blue", 0.01: "blue", 0.001: "blue"},
        height=3,
        aspect=1,
        sharey=True,
    )
    ...
    for assay_type_2 in assay_type_list_2:
        for fdr_1 in ["1", "01", "001"]:
            for fdr_2 in ["1", "01", "001"]:
                ...
                df["pred_cage_epi_cnn_1"] = df_1["pred_cage_epi_cnn"]
                df["pred_cage_epi_cnn_2"] = df_2["pred_cage_epi_cnn"]
                df["pred_cage_epi_cnn_logfc"] = np.log2(
                    (df_1["pred_cage_epi_cnn"] + 1) / (df_2["pred_cage_epi_cnn"] + 1)
                )
                ...
                # MSE
                mse_graphreg_all = np.mean(
                    (df["pred_cage_epi_graphreg_logfc"].values - df["true_cage_logfc"].values) ** 2
                )
                mse_graphreg_ex = np.mean(
                    (df_expressed["pred_cage_epi_graphreg_logfc"].values - df_expressed["true_cage_logfc"].values) ** 2
                )
                mse_graphreg_ex_int = np.mean(
                    (
                        df_expressed_interacting["pred_cage_epi_graphreg_logfc"].values
                ...
                mse_cnn_ex = np.mean(
                    (df_expressed["pred_cage_epi_cnn_logfc"].values - df_expressed["true_cage_logfc"].values) ** 2
                )
                mse_cnn_ex_int = np.mean(
                    (
                        df_expressed_interacting["pred_cage_epi_cnn_logfc"].values
                        - df_expressed_interacting["true_cage_logfc"].values
                    )
                    ** 2
                )
                ...
                r_graphreg_ex = np.corrcoef(
                    df_expressed["true_cage_logfc"].values,
                    df_expressed["pred_cage_epi_graphreg_logfc"],
                )[0, 1]
                r_graphreg_ex_int = np.corrcoef(
                    df_expressed_interacting["true_cage_logfc"].values,
                    df_expressed_interacting["pred_cage_epi_graphreg_logfc"],
                )[0, 1]

                r_cnn_all = np.corrcoef(df["true_cage_logfc"].values, df["pred_cage_epi_cnn_logfc"])[0, 1]
                r_cnn_ex = np.corrcoef(
                    df_expressed["true_cage_logfc"].values,
                    df_expressed["pred_cage_epi_cnn_logfc"],
                )[0, 1]
                r_cnn_ex_int = np.corrcoef(
                    df_expressed_interacting["true_cage_logfc"].values,
                    df_expressed_interacting["pred_cage_epi_cnn_logfc"],
                )[0, 1]
                ...
                norm = plt.Normalize(
                    df_expressed["n_contact_min_log2"].min(),
                    df_expressed["n_contact_min_log2"].max(),
                )
                sm = plt.cm.ScalarMappable(cmap="viridis_r", norm=norm)
                sm.set_array([])
                ax.get_legend().remove()
                ...
                ax.set_title(
                    "Epi-CNN | {}/{} | R = {:5.3f} | MSE = {:6.2f}".format(
                        cell_line_1, cell_line_2, r_cnn_ex, mse_cnn_ex
                    )
                )
                ...
                plt.tight_layout()
                plt.savefig(
                    "../figs/Epi-models/final/scatterplot/scatterplot_logfc_epi_cnn_"
                    + cell_line_1
                    + "_"
                    + assay_type_1
                    + "_FDR_"
                ...
                g = sns.scatterplot(
                    data=df_expressed,
                    x="true_cage_logfc",
                    y="pred_cage_epi_graphreg_logfc",
                    hue="n_contact_min_log2",
                    alpha=0.5,
                    palette=plt.cm.get_cmap("viridis_r"),
                    ax=ax,
                ...
                ax.figure.colorbar(sm, label="log2 (M + 1)")
                ax.set_title(
                    "Epi-GraphReg | {}/{} | R = {:5.3f} | MSE = {:6.2f}".format(
                ...
                plt.tight_layout()
                plt.savefig(
                    "../figs/Epi-models/final/scatterplot/scatterplot_logfc_epi_graphreg_"
                    + cell_line_1
                    + "_"
                    + assay_type_1
                    + "_FDR_"
...
g.fig.savefig(
    "../figs/Epi-models/final/swarmplot_logfc_R_" + cell_line_1 + "_to_" + cell_line_2 + "_check_3D_and_fdr.pdf",
    bbox_inches="tight",
)
g.fig.clf()
...
g.set_xticklabels(labels, rotation=45)
g.fig.savefig(
    "../figs/Epi-models/final/swarmplot_logfc_MSE_" + cell_line_1 + "_to_" + cell_line_2 + "_check_3D_and_fdr.pdf",
    bbox_inches="tight",
)
g.fig.clf()
...
plt.tight_layout()
plt.savefig(
    "../figs/Epi-models/final/scatterplot/scatterplot_delta_nll_sorted_hue_n_contact_"
    + cell_line
    + "_"
    + assay_type
    + "_FDR_"
...
df_expressed_sorted = df_expressed_sorted[
    (
        ...
        & (
            (df_expressed_sorted["nll_epi_graphreg"] < nll_threshod)
            | (df_expressed_sorted["nll_epi_cnn"] < nll_threshod)
        )
    )
]
...
sns.heatmap(
    df_expressed_sorted[["pred_cage_epi_graphreg"]],
    cmap="viridis_r",
    yticklabels=1,
    ax=ax4,
    vmax=max_cage,
...
sns.heatmap(
    df_expressed_sorted[["pred_cage_epi_cnn"]],
    cmap="viridis_r",
    yticklabels=1,
    ax=ax5,
    vmax=max_cage,
...
plt.tight_layout()
fig.savefig(
    "../figs/Epi-models/final/scatterplot/heatmap_best_genes_" + cell_line + "_" + assay_type + "_FDR_" + fdr + ".pdf",
    bbox_inches="tight",
)
