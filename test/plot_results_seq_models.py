...
df1 = pd.read_csv(
    data_path
    + "/results/csv/cage_prediction/seq_models/R_NLL_seq_e2e_models_"
    + cell_line
    + "_"
    + assay_type
    + "_FDR_"
...
for j in range(n_col):
    ...
    add_stat_annotation(
        ...
        hue="Method",
        box_pairs=[
            (("fft:no/dilated:yes", "Seq-GraphReg"), ("fft:no/dilated:yes", "Seq-CNN")),
            (("fft:no/dilated:no", "Seq-GraphReg"), ("fft:no/dilated:no", "Seq-CNN")),
            (
                ("fft:yes/dilated:yes", "Seq-GraphReg"),
                ("fft:yes/dilated:yes", "Seq-CNN"),
            ),
...
df1 = pd.read_csv(
    data_path + "/results/csv/cage_prediction/seq_models/R_NLL_basenji_" + cell_line + ".csv",
    sep="\t",
)
df = df.append(df1, ignore_index=True)
...
g = sns.catplot(
    ...
    aspect=1,
    sharey=False,
    palette={"Seq-GraphReg": "orange", "Seq-CNN": "deepskyblue", "Basenji": "green"},
)
g.set_xticklabels(rotation=15, horizontalalignment="right")
...
for j in range(n_col):
    ...
    add_stat_annotation(
        ...
        loc="inside",
        verbose=0,
        order=["All", "Expressed", "Interacting"],
        fontsize="x-large",
        comparisons_correction=None,
    )
...
for i in range(50):
    ...
    r_graphreg_ex = np.corrcoef(
        df_sub_expressed["true_cage_log2"].values,
        df_sub_expressed["pred_cage_seq_graphreg_log2"],
    )[0, 1]
    r_graphreg_ex_int = np.corrcoef(
        df_sub_interacting["true_cage_log2"].values,
        df_sub_interacting["pred_cage_seq_graphreg_log2"],
    )[0, 1]

    r_cnn_all = np.corrcoef(df_sub["true_cage_log2"].values, df_sub["pred_cage_seq_cnn_log2"])[0, 1]
    r_cnn_ex = np.corrcoef(
        df_sub_expressed["true_cage_log2"].values,
        df_sub_expressed["pred_cage_seq_cnn_log2"],
    )[0, 1]
    r_cnn_ex_int = np.corrcoef(
        df_sub_interacting["true_cage_log2"].values,
        df_sub_interacting["pred_cage_seq_cnn_log2"],
    )[0, 1]

    df_R_NLL = df_R_NLL.append(
        {
            "Cell": cell_line,
            "Method": "Seq-GraphReg",
            "Genes": "All",
            "Number of Genes": len(df_sub),
            "3D Data": assay_type,
            "FDR": qval_dict[fdr],
            "R": r_graphreg_all,
            "NLL": nll_graphreg_all_mean,
        },
        ignore_index=True,
    )
    df_R_NLL = df_R_NLL.append(
        {
            ...
            "Method": "Seq-GraphReg",
            "Genes": "Expressed",
            "Number of Genes": len(df_sub_expressed),
            "3D Data": assay_type,
            "FDR": qval_dict[fdr],
            "R": r_graphreg_ex,
            "NLL": nll_graphreg_ex_mean,
        },
        ignore_index=True,
    )
    df_R_NLL = df_R_NLL.append(
        {
            ...
            "Method": "Seq-GraphReg",
            "Genes": "Interacting",
            "Number of Genes": len(df_sub_interacting),
            "3D Data": assay_type,
            "FDR": qval_dict[fdr],
            "R": r_graphreg_ex_int,
            "NLL": nll_graphreg_ex_int_mean,
        },
        ignore_index=True,
    )
...
df["pred_cage_seq_graphreg_logfc"] = np.log2(
    (df_1["pred_cage_seq_graphreg"] + 1) / (df_2["pred_cage_seq_graphreg"] + 1)
)
...
for i in range(50):
    ...
    mse_graphreg_ex = np.mean(
        (df_sub_expressed["pred_cage_seq_graphreg_logfc"].values - df_sub_expressed["true_cage_logfc"].values) ** 2
    )
    mse_graphreg_ex_int = np.mean(
        (df_sub_interacting["pred_cage_seq_graphreg_logfc"].values - df_sub_interacting["true_cage_logfc"].values) ** 2
    )

    mse_cnn_all = np.mean((df_sub["pred_cage_seq_cnn_logfc"].values - df_sub["true_cage_logfc"].values) ** 2)
    mse_cnn_ex = np.mean(
        (df_sub_expressed["pred_cage_seq_cnn_logfc"].values - df_sub_expressed["true_cage_logfc"].values) ** 2
    )
    mse_cnn_ex_int = np.mean(
        (df_sub_interacting["pred_cage_seq_cnn_logfc"].values - df_sub_interacting["true_cage_logfc"].values) ** 2
    )

    # R
    r_graphreg_all = np.corrcoef(df_sub["true_cage_logfc"].values, df_sub["pred_cage_seq_graphreg_logfc"])[0, 1]
    r_graphreg_ex = np.corrcoef(
        df_sub_expressed["true_cage_logfc"].values,
        df_sub_expressed["pred_cage_seq_graphreg_logfc"],
    )[0, 1]
    r_graphreg_ex_int = np.corrcoef(
        df_sub_interacting["true_cage_logfc"].values,
        df_sub_interacting["pred_cage_seq_graphreg_logfc"],
    )[0, 1]

    r_cnn_all = np.corrcoef(df_sub["true_cage_logfc"].values, df_sub["pred_cage_seq_cnn_logfc"])[0, 1]
    r_cnn_ex = np.corrcoef(
        df_sub_expressed["true_cage_logfc"].values,
        df_sub_expressed["pred_cage_seq_cnn_logfc"],
    )[0, 1]
    r_cnn_ex_int = np.corrcoef(
        df_sub_interacting["true_cage_logfc"].values,
        df_sub_interacting["pred_cage_seq_cnn_logfc"],
    )[0, 1]

    df_R_MSE = df_R_MSE.append(
        {
            "Cell": cell_line_1 + " / " + cell_line_2,
            "Method": "Seq-GraphReg",
            "Genes": "All",
            "Number of Genes": len(df_sub),
            "3D Data": assay_type_1 + " / " + assay_type_2,
            "FDR": str(qval_dict[fdr_1]) + " / " + str(qval_dict[fdr_2]),
            "R": r_graphreg_all,
            "MSE": mse_graphreg_all,
        },
        ignore_index=True,
    )
    df_R_MSE = df_R_MSE.append(
...
plt.tight_layout()
plt.savefig(
    "../figs/Seq-models/final/boxplot_R_logfc_"
    + cell_line_1
    + "_"
    + assay_type_1
    + "_FDR_"
...
plt.tight_layout()
plt.savefig(
    "../figs/Seq-models/final/boxplot_MSE_logfc_"
    + cell_line_1
    + "_"
    + assay_type_1
    + "_FDR_"
...
for cell_line in ["K562", "K562", "K562", "K562", "K562", "GM12878", "hESC", "mESC"]:
    ...
    r_graphreg_ex = np.corrcoef(
        df_expressed["true_cage_log2"].values,
        df_expressed["pred_cage_seq_graphreg_log2"],
    )[0, 1]
    r_graphreg_ex_int = np.corrcoef(
        df_expressed_interacting["true_cage_log2"].values,
        df_expressed_interacting["pred_cage_seq_graphreg_log2"],
    )[0, 1]
    ...
    r_cnn_ex_int = np.corrcoef(
        df_expressed_interacting["true_cage_log2"].values,
        df_expressed_interacting["pred_cage_seq_cnn_log2"],
    )[0, 1]
    ...
    g = sns.scatterplot(
        ...
        hue="true_cage_log2",
        alpha=0.5,
        palette=plt.cm.get_cmap("viridis_r"),
        ax=ax,
    )
    ...
    if k == 0:
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_delta_nll_hue_cage_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_delta_nll_hue_cage_fft_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_delta_nll_hue_cage_nodilation_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_delta_nll_hue_cage_nodilation_fft_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_delta_nll_hue_cage_e2e_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
    ...
    ax.set_title(f"Seq-CNN | {cell_line} | R = {r_cnn_ex:5.3f} | NLL = {nll_cnn_ex_mean:6.2f}")
    ax.set_xlabel("log2 (true + 1)")
    ax.set_ylabel("log2 (pred + 1)")
    plt.tight_layout()
    if k == 0:
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_cnn_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_cnn_fft_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_cnn_nodilation_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_cnn_nodilation_fft_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_cnn_e2e_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
    ...
    ax.get_legend().remove()
    ax.figure.colorbar(sm, label="log2 (N + 1)")
    ax.set_title(f"Seq-GraphReg | {cell_line} | R = {r_graphreg_ex:5.3f} | NLL = {nll_graphreg_ex_mean:6.2f}")
    ax.set_xlabel("log2 (true + 1)")
    ax.set_ylabel("log2 (pred + 1)")
    plt.tight_layout()
    if k == 0:
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_graphreg_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_graphreg_fft_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_graphreg_nodilation_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_graphreg_nodilation_fft_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
        ...
        plt.savefig(
            "../figs/Seq-models/final/scatterplot/scatterplot_cage_vs_pred_seq_graphreg_e2e_"
            + cell_line
            + "_"
            + assay_type
            + "_FDR_"
    ...
    df_R_NLL = df_R_NLL.append(
        {
            ...
            "Train Mode": train_mode[k],
            "Genes": "Expressed",
            "Number of Genes": len(df_expressed),
            "3D Data": assay_type,
            "FDR": qval_dict[fdr],
            "R": r_graphreg_ex,
            "NLL": nll_graphreg_ex_mean,
    ...
    df_R_NLL = df_R_NLL.append(
        {
            ...
            "Train Mode": train_mode[k],
            "Genes": "Interacting",
            "Number of Genes": len(df_expressed_interacting),
            "3D Data": assay_type,
            "FDR": qval_dict[fdr],
            "R": r_graphreg_ex_int,
            "NLL": nll_graphreg_ex_int_mean,
...
g.fig.savefig(
    "../figs/Seq-models/final/swarmplot_R_K562_check_fft_and_dilation.pdf",
    bbox_inches="tight",
)
g.fig.clf()
...
g.set_xticklabels(labels, rotation=45)
g.fig.savefig(
    "../figs/Seq-models/final/swarmplot_NLL_K562_check_fft_and_dilation.pdf",
    bbox_inches="tight",
)
g.fig.clf()
...
for cell_line in ["K562", "GM12878"]:
    df_basenji = pd.read_csv(
        data_path + "/results/csv/cage_prediction/seq_models/cage_predictions_basenji_" + cell_line + ".csv",
        sep="\t",
    )
    if cell_line == "K562":
        df_basenji = df_basenji[
            ~df_basenji["chr"].isin(["chr3", "chr13", "chr8", "chr18", "chr7", "chr17", "chr4", "chr14"])
    ...
    df_expressed_interacting = df_basenji[
        ((df_basenji["true_cage"] >= 5) & (df_basenji["n_contact"] >= 1))
    ].reset_index()
    ...
    nll_basenji_ex_int = poisson_loss(
        df_expressed_interacting["true_cage"].values,
        df_expressed_interacting["pred_cage_basenji"].values,
    ).numpy()
    ...
    r_basenji_ex_int = np.corrcoef(
        df_expressed_interacting["true_cage_log2"].values,
        df_expressed_interacting["pred_cage_basenji_log2"],
    )[0, 1]
    ...
    df_R_NLL = df_R_NLL.append(
        {
            "Cell": cell_line,
            "Method": "Basenji",
            "Train Mode": "end to end",
            "Genes": "All",
            "Number of Genes": len(df_basenji),
            "3D Data": "none",
            "FDR": 1,
            "R": r_basenji_all,
            "NLL": nll_basenji_all_mean,
        },
        ignore_index=True,
    )
    df_R_NLL = df_R_NLL.append(
...
mse_graphreg_ex = np.mean(
    (df_expressed["pred_cage_seq_graphreg_logfc"].values - df_expressed["true_cage_logfc"].values) ** 2
)
mse_graphreg_ex_int = np.mean(
    (
        df_expressed_interacting["pred_cage_seq_graphreg_logfc"].values
        - df_expressed_interacting["true_cage_logfc"].values
    )
    ** 2
)
...
mse_cnn_ex_int = np.mean(
    (df_expressed_interacting["pred_cage_seq_cnn_logfc"].values - df_expressed_interacting["true_cage_logfc"].values)
    ** 2
)
...
r_graphreg_ex_int = np.corrcoef(
    df_expressed_interacting["true_cage_logfc"].values,
    df_expressed_interacting["pred_cage_seq_graphreg_logfc"],
)[0, 1]
...
r_cnn_ex_int = np.corrcoef(
    df_expressed_interacting["true_cage_logfc"].values,
    df_expressed_interacting["pred_cage_seq_cnn_logfc"],
)[0, 1]
...
g = sns.scatterplot(
    data=df_expressed,
    x="true_cage_logfc",
    y="pred_cage_seq_cnn_logfc",
    hue="n_contact_min_log2",
    alpha=0.5,
    palette=plt.cm.get_cmap("viridis_r"),
    ax=ax,
)
...
ax.set_title(f"Seq-CNN | {cell_line_1}/{cell_line_2} | R = {r_cnn_ex:5.3f} | MSE = {mse_cnn_ex:6.2f}")
...
plt.tight_layout()
plt.savefig(
    "../figs/Seq-models/final/scatterplot/scatterplot_logfc_seq_cnn_"
    + cell_line_1
    + "_"
    + assay_type_1
    + "_FDR_"
...
g = sns.scatterplot(
    data=df_expressed,
    x="true_cage_logfc",
    y="pred_cage_seq_graphreg_logfc",
    hue="n_contact_min_log2",
    alpha=0.5,
    palette=plt.cm.get_cmap("viridis_r"),
    ax=ax,
...
ax.figure.colorbar(sm, label="log2 (M + 1)")
ax.set_title(
    "Seq-GraphReg | {}/{} | R = {:5.3f} | MSE = {:6.2f}".format(
        cell_line_1, cell_line_2, r_graphreg_ex, mse_graphreg_ex
    )
)
ax.set_xlabel("log2 (FC true)", fontsize=20)
ax.set_ylabel("log2 (FC pred)", fontsize=20)
...
plt.tight_layout()
plt.savefig(
    "../figs/Seq-models/final/scatterplot/scatterplot_logfc_seq_graphreg_"
    + cell_line_1
    + "_"
    + assay_type_1
    + "_FDR_"
...
g.set_xticklabels(labels, rotation=45)
g.fig.savefig(
    "../figs/Seq-models/final/swarmplot_logfc_R_" + cell_line_1 + "_to_" + cell_line_2 + "_check_3D_and_fdr.pdf",
    bbox_inches="tight",
)
g.fig.clf()
...
g.set_xticklabels(labels, rotation=45)
g.fig.savefig(
    "../figs/Seq-models/final/swarmplot_logfc_MSE_" + cell_line_1 + "_to_" + cell_line_2 + "_check_3D_and_fdr.pdf",
    bbox_inches="tight",
)
g.fig.clf()
...
plt.tight_layout()
plt.savefig(
    "../figs/Seq-models/final/scatterplot/scatterplot_delta_nll_sorted_hue_n_contact_"
    + cell_line
    + "_"
    + assay_type
    + "_FDR_"
...
df_expressed_sorted["Gene Index"] = np.arange(len(df_expressed_sorted))
df_expressed_sorted = df_expressed_sorted[
    (
        (np.abs(df_expressed_sorted["delta_nll"]) > delta_nll_threshold)
        & (
            (df_expressed_sorted["nll_seq_graphreg"] < nll_threshod)
            | (df_expressed_sorted["nll_seq_cnn"] < nll_threshod)
        )
    )
]
...
sns.heatmap(
    df_expressed_sorted[["n_contact_log2"]],
    cmap="viridis_r",
    yticklabels=1,
    ax=ax2,
    cbar_kws={"orientation": "vertical"},
...
sns.heatmap(
    df_expressed_sorted[["pred_cage_seq_graphreg"]],
    cmap="viridis_r",
    yticklabels=1,
    ax=ax4,
    vmax=max_cage,
...
sns.heatmap(
    df_expressed_sorted[["pred_cage_seq_cnn"]],
    cmap="viridis_r",
    yticklabels=1,
    ax=ax5,
    vmax=max_cage,
...
plt.tight_layout()
fig.savefig(
    "../figs/Seq-models/final/scatterplot/heatmap_best_genes_" + cell_line + "_" + assay_type + "_FDR_" + fdr + ".pdf",
    bbox_inches="tight",
)
