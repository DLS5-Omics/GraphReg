...
def calculate_loss(
    ...
    if write_bw == True and organism == "human" and genome == "hg19":
        ...
        bw_y_true = pyBigWig.open(
            data_path
            + "/results/bigwig/cage_prediction/Epi-models/CAGE_"
            + cell_line_train
            + "_to_"
            + cell_line_test
            + "_valid_chr_"
        ...
        bw_y_pred_gat = pyBigWig.open(
            data_path
            + "/results/bigwig/cage_prediction/Epi-models/Epi-GraphReg_CAGE_pred_"
            + cell_line_train
            + "_to_"
            + cell_line_test
            + "_valid_chr_"
        ...
        bw_y_pred_cnn = pyBigWig.open(
            data_path
            + "/results/bigwig/cage_prediction/Epi-models/Epi-CNN_CAGE_pred_"
            + cell_line_train
            + "_to_"
            + cell_line_test
            + "_valid_chr_"
    ...
    for assay_type_train in assay_type_train_list:
        for assay_type_test in assay_type_test_list:
            for fdr_train in ["1"]:
                for fdr_test in ["1"]:
                    print(
                        "c: {}, assay_type_train: {}, assay_type_test: {}, fdr_train: {}, fdr_test: {}".format(
                            c, assay_type_train, assay_type_test, fdr_train, fdr_test
                        )
                    )
                    if prediction == True:
                        ...
                        for i in range(1, 1 + 10):
                            ...
                            if load_np == True:
                                y_gene = np.load(
                                    data_path
                                    + "/results/numpy/cage_prediction/true_cage_"
                                    + cell_line_train
                                    + "_to_"
                                    + cell_line_test
                                    + "_"
                                ...
                                y_hat_gene_gat = np.load(
                                    data_path
                                    + "/results/numpy/cage_prediction/Epi-GraphReg_predicted_cage_"
                                    + cell_line_train
                                    + "_to_"
                                    + cell_line_test
                                    + "_"
                                ...
                                y_hat_gene_cnn = np.load(
                                    data_path
                                    + "/results/numpy/cage_prediction/Epi-CNN_predicted_cage_"
                                    + cell_line_train
                                    + "_to_"
                                    + cell_line_test
                                    + "_"
                                ...
                                n_contacts = np.load(
                                    data_path
                                    + "/results/numpy/cage_prediction/n_contacts_"
                                    + cell_line_train
                                    + "_to_"
                                    + cell_line_test
                                    + "_"
                            ...
                            for j in range(4):
                                ...
                                idx = np.where(
                                    np.logical_and(
                                        n_contacts >= min_contact,
                                        y_gene >= min_expression,
                                    )
                                )[0]
                                ...
                                valid_rho_gat[i - 1, j] = np.corrcoef(
                                    np.log2(y_gene_idx + 1),
                                    np.log2(y_hat_gene_gat_idx + 1),
                                )[0, 1]
                                valid_sp_gat[i - 1, j] = spearmanr(
                                    np.log2(y_gene_idx + 1),
                                    np.log2(y_hat_gene_gat_idx + 1),
                                )[0]
                                valid_loss_cnn[i - 1, j] = poisson_loss(y_gene_idx, y_hat_gene_cnn_idx).numpy()
                                valid_rho_cnn[i - 1, j] = np.corrcoef(
                                    np.log2(y_gene_idx + 1),
                                    np.log2(y_hat_gene_cnn_idx + 1),
                                )[0, 1]
                                valid_sp_cnn[i - 1, j] = spearmanr(
                                    np.log2(y_gene_idx + 1),
                                    np.log2(y_hat_gene_cnn_idx + 1),
                                )[0]
                        ...
                        if save_R_NLL_to_csv:
                            ...
                            for i in range(1, 1 + 10):
                                ...
                                df = df.append(
                                    {
                                        "Cell_train": cell_line_train,
                                        "Cell_test": cell_line_test,
                                        "Method": "Epi-GraphReg",
                                        "Set": "All",
                                        "valid_chr": valid_chr_str,
                                        "test_chr": test_chr_str,
                                        "n_gene_test": n_gene[i - 1, 0],
                                        "3D_data_train": assay_type_train,
                                        "FDR_train": fdr_dict[fdr_train],
                                        "3D_data_test": assay_type_test,
                                        "FDR_test": fdr_dict[fdr_test],
                            ...
                            df.to_csv(
                                data_path
                                + "/results/csv/cage_prediction/cell_to_cell/R_NLL_epi_models_RPGC_"
                                + cell_line_train
                                + "_"
                                + assay_type_train
                                + "_FDR_"
                        ...
                        if plot_violin == True:
                            ...
                            plt.savefig(
                                "../figs/Epi-models/violinplot_" + cell_line_train + "_to_" + cell_line_test + ".png"
                            )
                        ...
                        if plot_box == True:
                            ...
                            plt.savefig(
                                "../figs/Epi-models/boxplot_" + cell_line_train + "_to_" + cell_line_test + "_CD.png"
                            )
                            ...
                            fig.tight_layout(rect=[0, 0, 1, 0.93])
                            plt.savefig(
                                "../figs/Epi-models/boxplot_" + cell_line_train + "_to_" + cell_line_test + "_all.png"
                            )
                        ...
                        if plot_scatter == True:
                            for i in range(1, 1 + 10):
                                ...
                                valid_rho_gat_n0 = np.corrcoef(
                                    np.log2(y_gene[idx_0] + 1),
                                    np.log2(y_hat_gene_gat[idx_0] + 1),
                                )[0, 1]
                                valid_loss_gat_n1 = poisson_loss(y_gene[idx_1], y_hat_gene_gat[idx_1]).numpy()
                                valid_rho_gat_n1 = np.corrcoef(
                                    np.log2(y_gene[idx_1] + 1),
                                    np.log2(y_hat_gene_gat[idx_1] + 1),
                                )[0, 1]

                                valid_loss_cnn_n0 = poisson_loss(y_gene[idx_0], y_hat_gene_cnn[idx_0]).numpy()
                                valid_rho_cnn_n0 = np.corrcoef(
                                    np.log2(y_gene[idx_0] + 1),
                                    np.log2(y_hat_gene_cnn[idx_0] + 1),
                                )[0, 1]
                                valid_loss_cnn_n1 = poisson_loss(y_gene[idx_1], y_hat_gene_cnn[idx_1]).numpy()
                                valid_rho_cnn_n1 = np.corrcoef(
                                    np.log2(y_gene[idx_1] + 1),
                                    np.log2(y_hat_gene_cnn[idx_1] + 1),
                                )[0, 1]
                                ...
                                plt.text(
                                    ...
                                    13,
                                    "n>0: R= "
                                    + f"{valid_rho_gat_n1:5.3f}"
                                    + ", NLL= "
                                    + str(np.float16(valid_loss_gat_n1)),
                                ...
                                plt.tight_layout()
                                plt.savefig(
                                    "../figs/Epi-models/scatter_plots/Epi-GraphReg_scatterplot_"
                                    + cell_line_train
                                    + "_to_"
                                    + cell_line_test
                                    + ".png"
                                ...
                                plt.text(
                                    ...
                                    13,
                                    "n>0: R= "
                                    + f"{valid_rho_cnn_n1:5.3f}"
                                    + ", NLL= "
                                    + str(np.float16(valid_loss_cnn_n1)),
                                    horizontalalignment="left",
                                    verticalalignment="top",
                                    bbox=props,
                                    fontsize=20,
                                ...
                                plt.tight_layout()
                                plt.savefig(
                                    "../figs/Epi-models/scatter_plots/Epi-CNN_scatterplot_"
                                    + cell_line_train
                                    + "_to_"
                                    + cell_line_test
                                    + ".png"
