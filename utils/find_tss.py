...
for chr in chr_list:
    ...
    filename_seqs = (
        data_path + "/data/csv/seqs_bed/" + organism + "/" + genome + "/" + resolution + "/sequences_" + chr + ".bed"
    )
    ...
    np.save(
        data_path + "/data/tss/" + organism + "/distal_regulation_group/" + genome + "/bin_start_" + chr,
        bin_start,
    )
    ...
    for i in range(n_tss):
        idx_tss = seq_dataframe.loc[
            (seq_dataframe["start"] <= protein_coding_tss["tss_1"][i])
            & (seq_dataframe["end"] > protein_coding_tss["tss_1"][i])
        ].index
        if len(idx_tss) == 0:
    ...
    np.save(
        data_path + "/data/tss/" + organism + "/distal_regulation_group/" + genome + "/tss_bins_" + chr,
        tss,
    )
    ...
    for i in range(n_bin):
        if tss[i] >= 1:  # if want to choose only bins with one tss: tss[i] == 1
            pos_tss1 = protein_coding_tss.loc[
                (seq_dataframe["start"][i] <= protein_coding_tss["tss_1"] - thr)
                & (seq_dataframe["end"][i] > protein_coding_tss["tss_1"] + thr)
            ]["tss_1"].values
            if len(pos_tss1) > 0:
                pos_tss1 = pos_tss1[0]
                gene_name1 = protein_coding_tss.loc[
                    (seq_dataframe["start"][i] <= protein_coding_tss["tss_1"] - thr)
                    & (seq_dataframe["end"][i] > protein_coding_tss["tss_1"] + thr)
                ]["gene"].values
    ...
    np.save(
        data_path + "/data/tss/" + organism + "/distal_regulation_group/" + genome + "/tss_pos_" + chr,
        pos_tss,
    )
    # np.save(data_path+'/data/tss/'+organism+'/'+genome+'/tss_gene_'+chr, gene_name)
    np.save(
        data_path + "/data/tss/" + organism + "/distal_regulation_group/" + genome + "/tss_gene_" + chr,
        gene_name,
    )
