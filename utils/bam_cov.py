...
# This code is copied from https://github.com/calico/basenji
...
def row_nzcols_set(m, ri, v):
    """Set row ri's nonzero columns to v."""
...
def distribute_multi_succint(
    ...
    for it in range(max_iterations):
        ...
        print(
            " Finished in %ds with %.3f change per multi-read" % (time.time() - t_it, iteration_change),
            flush=True,
        )
...
class GenomeCoverage:
    ...
    def distribute_multi(self, max_iterations=4, converge_t=0.05):
        ...
        for it in range(max_iterations):
            ...
            print(
                " Complete iteration in %ds with %.3f change per multi-read" % (time.time() - t_it, iteration_change),
                flush=True,
            )
    ...
    def learn_gc(self, fragment_sd=64, pseudocount=0.01, out_dir=None):
        ...
        for gsi in trainable_indexes:
            ...
            if gsm > last_gsm:
                print(
                    " GC training sequences: %d with %d multi-map positions." % (ntier, last_gsm),
                    flush=True,
                )
                ntier = 0
    ...
    def learn_shift_single(self, bam_file, shift_min=50, shift_max=350, out_dir=None):
        ...
        # mean normalize
        mean_count = 0.5 * counts_fwd[~multi_mask].mean(dtype="float64") + 0.5 * counts_rev[~multi_mask].mean(
            dtype="float64"
        )
        ...
        if genome_sorted:
            ...
            for read_id, ri in multi_read_index.items():
                if not np.isclose(multi_sum[ri], 1, rtol=1e-3):
                    ...
                    if not warned:
                        print(
                            "Multi-weighted coverage for ({},{}) {:f} != 1".format(
    ...
    def read_multi_bwa(
        ...
        for multi_align_str in multi_align_strings:
            ...
            if multi_strand == "+":
                ...
                print(
                    "Bad assumption of initial +- for BWA multimap position: %s" % multi_start,
                    file=sys.stderr,
                )
                exit(1)
