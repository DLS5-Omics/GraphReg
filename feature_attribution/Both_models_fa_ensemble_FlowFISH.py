...
def calculate_loss(
                ...
                if data_exist:
                    if tf.reduce_sum(tf.gather(tss_idx, idx)) > 0:
                        if (
                            ...
                            d = np.minimum(
                                np.abs(cut_flowfish["Gene.TSS"] - cut_flowfish["start"]),
                                np.abs(cut_flowfish["Gene.TSS"] - cut_flowfish["end"]),
                            ).values
                            ...
                            print(
                                "len significant_flowfish",
                                len(significant_flowfish_gene),
                            )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= n_enh:
                                ...
                                expr_change_percent_all = np.append(
                                    expr_change_percent_all,
                                    expr_change_percent_flowfish,
                                )
                                abc_all = np.append(abc_all, abc_flowfish)
                                sp_abc = np.append(
                                    sp_abc,
                                    spearmanr(abc_flowfish, expr_change_percent_flowfish)[0],
                                )
                                ...
                                (
                                    ...
                                    recall_abc,
                                    thresholds_abc,
                                ) = precision_recall_curve(Y_true, Y_pred_abc)
                                ...
                                print(
                                    "ABC AP score: ",
                                    average_precision_score(Y_true, Y_pred_abc),
                                )
                            ...
                            shap_values_e_cnn = np.load(
                                data_path
                                + "/results/numpy/feature_attribution/Epi-CNN_"
                                + saliency_method
                                + "_"
                                + cell_line
                                + "_"
                            ...
                            for s_idx, e_idx in zip(start_idx, end_idx):
                                ...
                                cnn_fa_gene = np.append(
                                    cnn_fa_gene,
                                    np.max(scores_cnn[mid_idx - L : mid_idx + L + 1]) / np.max(scores_cnn),
                                )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= n_enh:
                                ...
                                sp_e_cnn_deepshap = np.append(
                                    sp_e_cnn_deepshap,
                                    spearmanr(cnn_fa_gene, expr_change_percent_flowfish)[0],
                                )
                                ...
                                (
                                    ...
                                    recall_cnn,
                                    thresholds_cnn,
                                ) = precision_recall_curve(Y_true, Y_pred_cnn)
                                ap_e_cnn_deepshap = np.append(
                                    ap_e_cnn_deepshap,
                                    average_precision_score(Y_true, Y_pred_cnn),
                                )
                                auprc_e_cnn_deepshap = np.append(auprc_e_cnn_deepshap, auc(recall_cnn, precision_cnn))
                                print(
                                    "Epi-CNN Deepshap AP score: ",
                                    average_precision_score(Y_true, Y_pred_cnn),
                                )
                                print(
                                    "Epi-CNN Deepshap AUPRC score: ",
                                    auc(recall_cnn, precision_cnn),
                                )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= n_enh:
                                ...
                                ap_e_cnn_saliency = np.append(
                                    ap_e_cnn_saliency,
                                    average_precision_score(Y_true, Y_pred_cnn),
                                )
                                auprc_e_cnn_saliency = np.append(auprc_e_cnn_saliency, auc(recall_cnn, precision_cnn))
                                print(
                                    "Epi-CNN Saliency AP score: ",
                                    average_precision_score(Y_true, Y_pred_cnn),
                                )
                                print(
                                    "Epi-CNN Saliency AUPRC score: ",
                                    auc(recall_cnn, precision_cnn),
                                )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= n_enh:
                                ...
                                ap_s_cnn_saliency = np.append(
                                    ap_s_cnn_saliency,
                                    average_precision_score(Y_true, Y_pred_cnn),
                                )
                                auprc_s_cnn_saliency = np.append(auprc_s_cnn_saliency, auc(recall_cnn, precision_cnn))
                                print(
                                    "Seq-CNN Saliency AP score: ",
                                    average_precision_score(Y_true, Y_pred_cnn),
                                )
                                print(
                                    "Seq-CNN Saliency AUPRC score: ",
                                    auc(recall_cnn, precision_cnn),
                                )
                            ...
                            saliency_method = "deepshap"
                            shap_values_e_graphreg = np.load(
                                data_path
                                + "/results/numpy/feature_attribution/Epi-GraphReg_"
                                + saliency_method
                                + "_"
                                + cell_line
                                + "_"
                            ...
                            print(
                                "Epi-GraphReg deepshap: ",
                                scores_graphreg.shape,
                                np.min(scores_graphreg),
                                np.max(scores_graphreg),
                                np.mean(scores_graphreg),
                            )
                            ...
                            for s_idx, e_idx in zip(start_idx, end_idx):
                                cnt = cnt + 1
                                mid_idx = int((s_idx + e_idx) / 2)
                                graphreg_fa_gene = np.append(
                                    graphreg_fa_gene,
                                    np.max(scores_graphreg[mid_idx - L : mid_idx + L + 1]) / np.max(scores_graphreg),
                                )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= n_enh:
                                ...
                                sp_e_graphreg_deepshap = np.append(
                                    sp_e_graphreg_deepshap,
                                    spearmanr(graphreg_fa_gene, expr_change_percent_flowfish)[0],
                                )
                                ...
                                (
                                    ...
                                    recall_graphreg,
                                    thresholds_graphreg,
                                ) = precision_recall_curve(Y_true, Y_pred_graphreg)
                                ap_e_graphreg_deepshap = np.append(
                                    ap_e_graphreg_deepshap,
                                    average_precision_score(Y_true, Y_pred_graphreg),
                                )
                                auprc_e_graphreg_deepshap = np.append(
                                    auprc_e_graphreg_deepshap,
                                    auc(recall_graphreg, precision_graphreg),
                                )
                                print(
                                    "Epi-GraphReg Deepshap AP score: ",
                                    average_precision_score(Y_true, Y_pred_graphreg),
                                )
                                print(
                                    "Epi-GraphReg Deepshap AUPRC score: ",
                                    auc(recall_graphreg, precision_graphreg),
                                )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= n_enh:
                                ...
                                sp_e_graphreg_saliency = np.append(
                                    sp_e_graphreg_saliency,
                                    spearmanr(graphreg_fa_gene, expr_change_percent_flowfish)[0],
                                )
                                Y_pred_graphreg = graphreg_fa_gene
                                (
                                ...
                                ap_e_graphreg_saliency = np.append(
                                    ap_e_graphreg_saliency,
                                    average_precision_score(Y_true, Y_pred_graphreg),
                                )
                                auprc_e_graphreg_saliency = np.append(
                                    auprc_e_graphreg_saliency,
                                    auc(recall_graphreg, precision_graphreg),
                                )
                                print(
                                    "Epi-GraphReg Saliency AP score: ",
                                    average_precision_score(Y_true, Y_pred_graphreg),
                                )
                                print(
                                    "Epi-GraphReg Saliency AUPRC score: ",
                                    auc(recall_graphreg, precision_graphreg),
                                )
                            ...
                            saliency_method = "saliency"
                            shap_values_s_graphreg = np.load(
                                data_path
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= n_enh:
                                ...
                                sp_s_graphreg_saliency = np.append(
                                    sp_s_graphreg_saliency,
                                    spearmanr(graphreg_fa_gene, expr_change_percent_flowfish)[0],
                                )
                                Y_pred_graphreg = graphreg_fa_gene
                                (
                                ...
                                ap_s_graphreg_saliency = np.append(
                                    ap_s_graphreg_saliency,
                                    average_precision_score(Y_true, Y_pred_graphreg),
                                )
                                auprc_s_graphreg_saliency = np.append(
                                    auprc_s_graphreg_saliency,
                                    auc(recall_graphreg, precision_graphreg),
                                )
                                print(
                                    "Seq-GraphReg Saliency AP score: ",
                                    average_precision_score(Y_true, Y_pred_graphreg),
                                )
                                print(
                                    "Seq-GraphReg Saliency AUPRC score: ",
                                    auc(recall_graphreg, precision_graphreg),
                                )
    ...
    return (
        ...
        auprc_e_graphreg_saliency,
        auprc_s_graphreg_saliency,
        auprc_e_cnn_deepshap,
        auprc_e_cnn_saliency,
        auprc_s_cnn_saliency,
        ap_abc,
        ap_e_graphreg_deepshap,
        ap_e_graphreg_saliency,
        ap_s_graphreg_saliency,
        ...
        ap_s_cnn_saliency,
        abc_all,
        e_graphreg_fa_all_deepshap,
        e_graphreg_fa_all_saliency,
        s_graphreg_fa_all_saliency,
        e_cnn_fa_all_deepshap,
        e_cnn_fa_all_saliency,
        s_cnn_fa_all_saliency,
        expr_change_percent_all,
        significant_flowfish,
        D,
        df_with_enhancer_preds,
    )
...
data_frame_flowfish = data_frame_flowfish[
    np.minimum(
        np.abs(data_frame_flowfish["Gene.TSS"] - data_frame_flowfish["start"]),
        np.abs(data_frame_flowfish["Gene.TSS"] - data_frame_flowfish["end"]),
    )
    > dist
]
...
bp = ax.boxplot(SP_list[0], positions=[1], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="lightgreen")  # deepskyblue
bp = ax.boxplot(SP_list[1:6:2], positions=[2, 4, 6], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="orange")  # deepskyblue
bp = ax.boxplot(SP_list[2:7:2], positions=[3, 5, 7], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
...
fig.tight_layout()
ax = fig.add_subplot(111)
bp = ax.boxplot(aucpr_list[0], positions=[1], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="lightgreen")  # deepskyblue
bp = ax.boxplot(aucpr_list[1:6:2], positions=[2, 4, 6], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="orange")  # deepskyblue
bp = ax.boxplot(aucpr_list[2:7:2], positions=[3, 5, 7], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="deepskyblue")  # deepskyblue
    ...
    df = df.append(
        {
            "Method": "GraphReg",
            "FA": "Epi/DeepSHAP",
            "auPR": auprc_e_graphreg_deepshap[i],
            "ID": i,
        },
        ignore_index=True,
    )
    ...
    df = df.append(
        {
            "Method": "GraphReg",
            "FA": "Epi/Saliency",
            "auPR": auprc_e_graphreg_saliency[i],
            "ID": i,
        },
        ignore_index=True,
    )
    ...
    df = df.append(
        {
            "Method": "GraphReg",
            "FA": "Seq/Saliency",
            "auPR": auprc_s_graphreg_saliency[i],
            "ID": i,
        },
        ignore_index=True,
    )
...
b = sns.boxplot(
    ...
    data=df,
    hue="Method",
    palette={"GraphReg": "orange", "CNN": "deepskyblue", "ABC": "lightgreen"},
    order=["Epi/DeepSHAP", "Epi/Saliency", "Seq/Saliency", "N/A"],
    ax=ax1,
)
...
fig.tight_layout()
ax = fig.add_subplot(111)
bp = ax.boxplot(ap_list[0], positions=[1], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="lightgreen")  # deepskyblue
bp = ax.boxplot(ap_list[1:6:2], positions=[2, 4, 6], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="orange")  # deepskyblue
bp = ax.boxplot(ap_list[2:7:2], positions=[3, 5, 7], widths=0.4, patch_artist=True)
for element in ["boxes", "whiskers", "fliers", "means", "medians", "caps"]:
    plt.setp(bp[element], color="black")
for patch in bp["boxes"]:
    patch.set(facecolor="deepskyblue")  # deepskyblue
...
plt.scatter(abc_all[idx_0], expr_change_percent_all[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(abc_all[idx_2], expr_change_percent_all[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(abc_all[idx_1], expr_change_percent_all[idx_1], color="red", s=80, alpha=0.5)
...
plt.text(
    2e-6,
    0.45,
    r"$\rho$ = " + str(spearmanr(abc_all[idx_1], expr_change_percent_all[idx_1])[0].astype(np.float16)) + " (Decrease)",
...
plt.figure(figsize=(15, 15))
plt.scatter(
    e_graphreg_fa_all_deepshap[idx_0],
    expr_change_percent_all[idx_0],
    color="grey",
    s=80,
    alpha=0.5,
)
plt.scatter(
    e_graphreg_fa_all_deepshap[idx_2],
    expr_change_percent_all[idx_2],
    color="blue",
    s=80,
    alpha=0.5,
)
plt.scatter(
    e_graphreg_fa_all_deepshap[idx_1],
    expr_change_percent_all[idx_1],
    color="red",
    s=80,
    alpha=0.5,
)
...
plt.text(
    ...
    0.45,
    r"$\rho$ = "
    + str(spearmanr(e_graphreg_fa_all_deepshap[idx_1], expr_change_percent_all[idx_1])[0].astype(np.float16))
    + " (Decrease)",
    horizontalalignment="left",
    verticalalignment="top",
    bbox=props,
...
plt.figure(figsize=(15, 15))
plt.scatter(
    e_graphreg_fa_all_saliency[idx_0],
    expr_change_percent_all[idx_0],
    color="grey",
    s=80,
    alpha=0.5,
...
plt.scatter(
    e_graphreg_fa_all_saliency[idx_2],
    expr_change_percent_all[idx_2],
    color="blue",
    s=80,
    alpha=0.5,
...
plt.scatter(
    e_graphreg_fa_all_saliency[idx_1],
    expr_change_percent_all[idx_1],
    color="red",
    s=80,
    alpha=0.5,
...
plt.text(
    ...
    0.45,
    r"$\rho$ = "
    + str(spearmanr(e_graphreg_fa_all_saliency[idx_1], expr_change_percent_all[idx_1])[0].astype(np.float16))
    + " (Decrease)",
    horizontalalignment="left",
    verticalalignment="top",
    bbox=props,
...
plt.figure(figsize=(15, 15))
plt.scatter(
    s_graphreg_fa_all_saliency[idx_0],
    expr_change_percent_all[idx_0],
    color="grey",
    s=80,
    alpha=0.5,
...
plt.scatter(
    s_graphreg_fa_all_saliency[idx_2],
    expr_change_percent_all[idx_2],
    color="blue",
    s=80,
    alpha=0.5,
...
plt.scatter(
    s_graphreg_fa_all_saliency[idx_1],
    expr_change_percent_all[idx_1],
    color="red",
    s=80,
    alpha=0.5,
...
plt.text(
    ...
    0.45,
    r"$\rho$ = "
    + str(spearmanr(s_graphreg_fa_all_saliency[idx_1], expr_change_percent_all[idx_1])[0].astype(np.float16))
    + " (Decrease)",
    horizontalalignment="left",
    verticalalignment="top",
    bbox=props,
...
plt.text(
    ...
    0.45,
    r"$\rho$ = "
    + str(spearmanr(e_cnn_fa_all_deepshap[idx_1], expr_change_percent_all[idx_1])[0].astype(np.float16))
    + " (Decrease)",
    horizontalalignment="left",
    verticalalignment="top",
    bbox=props,
...
plt.text(
    ...
    0.45,
    r"$\rho$ = "
    + str(spearmanr(e_cnn_fa_all_saliency[idx_1], expr_change_percent_all[idx_1])[0].astype(np.float16))
    + " (Decrease)",
    horizontalalignment="left",
    verticalalignment="top",
    bbox=props,
...
plt.text(
    ...
    0.45,
    r"$\rho$ = "
    + str(spearmanr(s_cnn_fa_all_saliency[idx_1], expr_change_percent_all[idx_1])[0].astype(np.float16))
    + " (Decrease)",
    horizontalalignment="left",
    verticalalignment="top",
    bbox=props,
...
plt.figure(figsize=(15, 15))
plt.scatter(D[idx_0], abc_all[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(D[idx_2], abc_all[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(D[idx_1], abc_all[idx_1], color="red", s=80, alpha=0.5)
plt.title("DE-G Pairs", fontsize=40)
...
plt.figure(figsize=(15, 15))
plt.scatter(D[idx_0], e_graphreg_fa_all_deepshap[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(D[idx_2], e_graphreg_fa_all_deepshap[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(D[idx_1], e_graphreg_fa_all_deepshap[idx_1], color="red", s=80, alpha=0.5)
plt.title("DE-G Pairs", fontsize=40)
plt.xlabel("Distance (bp)", fontsize=40)
...
plt.figure(figsize=(15, 15))
plt.scatter(D[idx_0], e_graphreg_fa_all_saliency[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(D[idx_2], e_graphreg_fa_all_saliency[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(D[idx_1], e_graphreg_fa_all_saliency[idx_1], color="red", s=80, alpha=0.5)
plt.title("DE-G Pairs", fontsize=40)
plt.xlabel("Distance (bp)", fontsize=40)
...
plt.figure(figsize=(15, 15))
plt.scatter(D[idx_0], s_graphreg_fa_all_saliency[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(D[idx_2], s_graphreg_fa_all_saliency[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(D[idx_1], s_graphreg_fa_all_saliency[idx_1], color="red", s=80, alpha=0.5)
plt.title("DE-G Pairs", fontsize=40)
plt.xlabel("Distance (bp)", fontsize=40)
...
plt.figure(figsize=(15, 15))
plt.scatter(D[idx_0], e_cnn_fa_all_deepshap[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(D[idx_2], e_cnn_fa_all_deepshap[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(D[idx_1], e_cnn_fa_all_deepshap[idx_1], color="red", s=80, alpha=0.5)
plt.title("DE-G Pairs", fontsize=40)
plt.xlabel("Distance (bp)", fontsize=40)
...
plt.figure(figsize=(15, 15))
plt.scatter(D[idx_0], e_cnn_fa_all_saliency[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(D[idx_2], e_cnn_fa_all_saliency[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(D[idx_1], e_cnn_fa_all_saliency[idx_1], color="red", s=80, alpha=0.5)
plt.title("DE-G Pairs", fontsize=40)
plt.xlabel("Distance (bp)", fontsize=40)
...
plt.figure(figsize=(15, 15))
plt.scatter(D[idx_0], s_cnn_fa_all_saliency[idx_0], color="grey", s=80, alpha=0.5)
plt.scatter(D[idx_2], s_cnn_fa_all_saliency[idx_2], color="blue", s=80, alpha=0.5)
plt.scatter(D[idx_1], s_cnn_fa_all_saliency[idx_1], color="red", s=80, alpha=0.5)
plt.title("DE-G Pairs", fontsize=40)
plt.xlabel("Distance (bp)", fontsize=40)
...
(
    precision_e_graphreg_deepshap,
    recall_e_graphreg_deepshap,
    thresholds_e_graphreg_deepshap,
) = precision_recall_curve(Y_true, Y_pred_gat)
...
(
    precision_e_cnn_deepshap,
    recall_e_cnn_deepshap,
    thresholds_e_cnn_deepshap,
) = precision_recall_curve(Y_true, Y_pred_cnn)
...
(
    precision_e_graphreg_saliency,
    recall_e_graphreg_saliency,
    thresholds_e_graphreg_saliency,
) = precision_recall_curve(Y_true, Y_pred_gat)
...
(
    precision_e_cnn_saliency,
    recall_e_cnn_saliency,
    thresholds_e_cnn_saliency,
) = precision_recall_curve(Y_true, Y_pred_cnn)
...
(
    precision_s_graphreg_saliency,
    recall_s_graphreg_saliency,
    thresholds_s_graphreg_saliency,
) = precision_recall_curve(Y_true, Y_pred_gat)
...
(
    precision_s_cnn_saliency,
    recall_s_cnn_saliency,
    thresholds_s_cnn_saliency,
) = precision_recall_curve(Y_true, Y_pred_cnn)
...
plt.plot(
    ...
    color="lightgreen",
    linewidth=3,
    label="ABC: AUC = " + str(auprc_abc_all.astype(np.float16)),
)
plt.plot(
    recall_e_graphreg_deepshap,
    precision_e_graphreg_deepshap,
    color="orange",
    linewidth=3,
    label="Epi-GraphReg (DeepSHAP): AUC = " + str(auprc_e_graphreg_all_deepshap.astype(np.float16)),
)
plt.plot(
    recall_e_graphreg_saliency,
    precision_e_graphreg_saliency,
    ...
    linewidth=3,
    linestyle="--",
    label="Epi-GraphReg (Saliency): AUC = " + str(auprc_e_graphreg_all_saliency.astype(np.float16)),
)
plt.plot(
    recall_s_graphreg_saliency,
    precision_s_graphreg_saliency,
    ...
    linewidth=3,
    linestyle=":",
    label="Seq-GraphReg (Saliency): AUC = " + str(auprc_s_graphreg_all_saliency.astype(np.float16)),
)
...
plt.plot(
    ...
    color="deepskyblue",
    linewidth=3,
    label="Epi-CNN (DeepSHAP): AUC = " + str(auprc_e_cnn_all_deepshap.astype(np.float16)),
)
plt.plot(
    recall_e_cnn_saliency,
    precision_e_cnn_saliency,
    ...
    linewidth=3,
    linestyle="--",
    label="Epi-CNN (Saliency): AUC = " + str(auprc_e_cnn_all_saliency.astype(np.float16)),
)
plt.plot(
    recall_s_cnn_saliency,
    precision_s_cnn_saliency,
    ...
    linewidth=3,
    linestyle=":",
    label="Seq-CNN (Saliency): AUC = " + str(auprc_s_cnn_all_saliency.astype(np.float16)),
)
...
##### SP ######
print(
    "ABC SP for significants: ",
    spearmanr(abc_all[idx], expr_change_percent_all[idx])[0],
)
print(
    "Epi-GraphReg (deepshap) SP for significants: ",
    spearmanr(e_graphreg_fa_all_deepshap[idx], expr_change_percent_all[idx])[0],
)
print(
    "Epi-GraphReg (saliency) SP for significants: ",
    spearmanr(e_graphreg_fa_all_saliency[idx], expr_change_percent_all[idx])[0],
)
print(
    "Seq-GraphReg (saliency) SP for significants: ",
    spearmanr(s_graphreg_fa_all_saliency[idx], expr_change_percent_all[idx])[0],
)
print(
    "Epi-CNN (deepshap) SP for significants: ",
    spearmanr(e_cnn_fa_all_deepshap[idx], expr_change_percent_all[idx])[0],
)
print(
    "Epi-CNN (saliency) SP for significants: ",
    spearmanr(e_cnn_fa_all_saliency[idx], expr_change_percent_all[idx])[0],
)
print(
    "Seq-CNN (saliency) SP for significants: ",
    spearmanr(s_cnn_fa_all_saliency[idx], expr_change_percent_all[idx])[0],
)
