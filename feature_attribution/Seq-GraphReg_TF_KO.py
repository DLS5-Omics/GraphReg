...
def calculate_loss(
            ...
            if data_exist:
                if tf.reduce_sum(tf.gather(tss_idx, idx)) > 0:
                    ...
                    for i_tf in range(len(TFs)):
                        TF_positions_df_sub = TF_positions_df[
                            (
                                (TF_positions_df["TF"] == TFs[i_tf])
                                & (TF_positions_df["chr"] == chrm)
                                & (TF_positions_df["start"] > pos[0])
                                & (TF_positions_df["end"] < pos[0] + 6000000)
                            )
                        ...
                        if len(TF_positions_df_sub) > 0:
                            ...
                            for ii in range(len(TF_positions_df_sub)):
                                ...
                                if j % 100000 < 100000 - 20:
                                    ...
                                    seq_np[k, start_mut_seq:end_mut_seq, :] = 0.0
                        ...
                        for j in range(len(tss_pos_1)):
                            ...
                            y_hat_gene_gat_ko_list[i_tf] = np.append(
                                y_hat_gene_gat_ko_list[i_tf], y_hat_gat_ko_[idx_tss]
                            )
                            y_hat_gene_cnn_ko_list[i_tf] = np.append(
                                y_hat_gene_cnn_ko_list[i_tf], y_hat_cnn_ko_[idx_tss]
                            )
    ...
    return (
        y_gene,
        y_hat_gene_gat,
        y_hat_gene_gat_ko_list,
        y_hat_gene_cnn,
        y_hat_gene_cnn_ko_list,
        chr_pos,
        gene_pos,
        gene_names,
        n_contacts,
...
for k in range(len(TFs)):
    df_GraphReg_dict[k].to_csv(
        data_path + "/results/csv/insilico_TF_KO/Seq-GraphReg_TF_KO_" + TFs[k] + ".tsv",
        sep="\t",
    )
    df_CNN_dict[k].to_csv(
        data_path + "/results/csv/insilico_TF_KO/Seq-CNN_TF_KO_" + TFs[k] + ".tsv",
        sep="\t",
    )
...
for k in range(len(TFs)):
    ...
    sample_df_GraphReg_deseq = (
        pd.DataFrame({"samplename": df_GraphReg_deseq.columns})
        .query('samplename != "id"')
    ...
    dds = py_DESeq2(
        count_matrix=df_GraphReg_deseq,
    ...
    res.to_csv(
        data_path + "/results/csv/CRISPRi_K562_DESeq_results/" + TFs[k] + "_KO_GraphReg.tsv",
        sep="\t",
    )
    ...
    sample_df_CNN_deseq = (
        pd.DataFrame({"samplename": df_CNN_deseq.columns})
        .query('samplename != "id"')
        .assign(sample=lambda d: d.samplename.str.extract("([AB])_", expand=False))
        .assign(replicate=lambda d: d.samplename.str.extract("_([123])", expand=False))
    )
    ...
    res = res.sort_values(by=["pvalue"]).reset_index(drop=True)
    res.to_csv(
        data_path + "/results/csv/CRISPRi_K562_DESeq_results/" + TFs[k] + "_KO_CNN.tsv",
        sep="\t",
    )
...
for k in range(len(TFs)):
    df_true = pd.read_csv(
        data_path + "/results/csv/CRISPRi_K562_DESeq_results/" + TFs[k] + "_KO.tsv",
        sep="\t",
    )
    df_GraphReg_deseq = pd.read_csv(
        data_path + "/results/csv/CRISPRi_K562_DESeq_results/" + TFs[k] + "_KO_GraphReg.tsv",
        sep="\t",
    )
    df_CNN_deseq = pd.read_csv(
        data_path + "/results/csv/CRISPRi_K562_DESeq_results/" + TFs[k] + "_KO_CNN.tsv",
        sep="\t",
    )
    ...
    for j in range(N):
        ...
        df.loc[genes_shared[j], "true_CAGE"] = df_GraphReg[df_GraphReg["gene_name"] == genes_shared[j]][
            "True_CAGE"
        ].values[0]
        df.loc[genes_shared[j], "n_contacts"] = df_GraphReg[df_GraphReg["gene_name"] == genes_shared[j]][
            "n_contact"
        ].values[0]

        df.loc[genes_shared[j], "baseMean_true"] = df_true[df_true["gene_name"] == genes_shared[j]]["baseMean"].values[
            0
        ]
        df.loc[genes_shared[j], "log2FoldChange_true"] = df_true[df_true["gene_name"] == genes_shared[j]][
            "log2FoldChange"
        ].values[0]
        df.loc[genes_shared[j], "pvalue_true"] = df_true[df_true["gene_name"] == genes_shared[j]]["pvalue"].values[0]

        df.loc[genes_shared[j], "baseMean_GraphReg"] = df_GraphReg_deseq[df_GraphReg_deseq["id"] == genes_shared[j]][
            "baseMean"
        ].values[0]
        df.loc[genes_shared[j], "log2FoldChange_GraphReg"] = df_GraphReg_deseq[
            df_GraphReg_deseq["id"] == genes_shared[j]
        ]["log2FoldChange"].values[0]
        df.loc[genes_shared[j], "pvalue_GraphReg"] = df_GraphReg_deseq[df_GraphReg_deseq["id"] == genes_shared[j]][
            "pvalue"
        ].values[0]

        df.loc[genes_shared[j], "baseMean_CNN"] = df_CNN_deseq[df_CNN_deseq["id"] == genes_shared[j]][
            "baseMean"
        ].values[0]
        df.loc[genes_shared[j], "log2FoldChange_CNN"] = df_CNN_deseq[df_CNN_deseq["id"] == genes_shared[j]][
            "log2FoldChange"
        ].values[0]
...
for k in range(len(TFs)):
    ...
    _, pvals_adjusted, _, _ = fdr.multipletests(
        df["pvalue_true"].values, alpha=p_adj_thr, method="fdr_bh", is_sorted=False
    )
    df["pvalue_true"] = pvals_adjusted

    df_pos = df[
        (
            (df["true_CAGE"] >= CAGE_min)
            & (df["n_contacts"] >= n_min)
            & (df["pvalue_true"] <= p_adj_thr)
            & (df["log2FoldChange_true"] < 0)
        )
    ]
    df_pos["class"] = 1
    ...
    if len(df_pos) >= 200:
        ...
        df_sns = df_sns.append(
            {
                "Mean": np.mean(df_pos_gr["log2FoldChange_true"].values),
                "Fraction": len(df_pos_gr[df_pos_gr["log2FoldChange_true"] < 0]) / N,
                "Method": "Seq-GraphReg",
                "TF_KO": TFs[k],
            },
            ignore_index=True,
        ...
        df_sns = df_sns.append(
            {
                "Mean": np.mean(df_pos_cnn["log2FoldChange_true"].values),
                "Fraction": len(df_pos_cnn[df_pos_cnn["log2FoldChange_true"] < 0]) / N,
                "Method": "Seq-CNN",
                "TF_KO": TFs[k],
            },
            ignore_index=True,
        ...
        df_sns = df_sns.append(
            {
                "Mean": np.mean(df_sub["log2FoldChange_true"].values),
                "Fraction": len(df_sub[df_sub["log2FoldChange_true"] < 0]) / len(df_sub),
                "Method": "Baseline",
                "TF_KO": TFs[k],
            },
            ignore_index=True,
...
b = sns.boxplot(
    ...
    y="Mean",
    data=df_sns,
    palette={"Seq-GraphReg": "orange", "Seq-CNN": "deepskyblue", "Baseline": "green"},
    ax=ax1,
)
add_stat_annotation(
    ax1,
    ...
    y="Mean",
    box_pairs=[
        (("Seq-GraphReg"), ("Seq-CNN")),
        (("Seq-GraphReg"), ("Baseline")),
        (("Seq-CNN"), ("Baseline")),
    ],
    test="Wilcoxon",
    text_format="star",
...
b = sns.swarmplot(x="Method", y="Mean", data=df_sns, color="black", alpha=0.3, size=8, ax=ax1)
...
b = sns.swarmplot(x="Method", y="Fraction", data=df_sns, color="black", alpha=0.3, size=8, ax=ax1)
ax1.yaxis.set_tick_params(labelsize=20)
ax1.xaxis.set_tick_params(labelsize=20)
ax1.set_title(r"$n \geq$" + str(n_min), fontsize=20)
b.set_xlabel("", fontsize=20)
...
ax.set_title(
    "Top 20 predicted downregulated genes by Seq-GraphReg when n >= "
    + str(n_min)
    + ", precision = "
    + "%.2f" % (np.sum(mat_gr_all < 0) / len(mat_gr_all.ravel())),
    fontsize=30,
)
ax.set_yticklabels(ax.get_ymajorticklabels(), fontsize=20, rotation=0)
ax.set_xticklabels(ax.get_xmajorticklabels(), fontsize=20, rotation=90)
...
ax.set_title(
    "Top 20 predicted downregulated genes by Seq-CNN when n >= "
    + str(n_min)
    + ", precision = "
    + "%.2f" % (np.sum(mat_cnn_all < 0) / len(mat_cnn_all.ravel())),
    fontsize=30,
)
ax.set_yticklabels(ax.get_ymajorticklabels(), fontsize=20, rotation=0)
ax.set_xticklabels(ax.get_xmajorticklabels(), fontsize=20, rotation=90)
...
g = sns.scatterplot(
    data=df_sub,
    x="log2FoldChange_true",
    y="log2FoldChange_GraphReg",
    alpha=0.5,
