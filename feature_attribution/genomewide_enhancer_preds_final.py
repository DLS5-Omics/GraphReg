...
df_enhancers = pd.read_csv(
    data_path
    + f"/results/csv/distal_reg_paper/EG_features/{cell_line}/FullModel/{cell_line}_AllFeatures_NAfilled.FullModel.tsv",
    delimiter="\t",
)
...
for chr in chr_list:
    ...
    if len(idx_test) > 0:
        ...
        with open(
            data_path + f"/results/csv/distal_reg_paper/CRISPR_ensemble/models/final/model_{model}_test_{chr}.pkl",
            "rb",
        ) as f:
            clf = pickle.load(f)
...
if save_to_csv:
    df_enhancers.to_csv(
        data_path
        + f"/results/csv/distal_reg_paper/EG_features/{cell_line}/FullModel/{cell_line}_AllFeatures_NAfilled.FullModel_withPreds.tsv",
        sep="\t",
        index=False,
    )
...
features_list = [
    ...
    "normalizedH3K27ac_enh",
    "STARRseqABC",
    "activityEnhDNaseOnlyAvgHicTrack1_squared",
    "ABCDenominator",
]
...
features_list = [
    ...
    "ABCDenominator",
    "activity_enh_squared",
    "3DContactAvgHicTrack1",
    "ubiquitousExpressedGene",
    "ABCScoreDNaseOnlyAvgHicTrack1",
    "3DContact",
]
...
features_list = [
    ...
    "ABCScoreDNaseOnlyAvgHicTrack1",
    "3DContact",
    "activityPromDNaseOnlyAvgHicTrack1",
    "H3K4me3_p_grad_min_L_8",
    "numTSSEnhGene",
    "H3K27ac_e_grad_max_L_8",
    "H3K27ac_e_grad_min_L_8",
...
if save_to_csv:
    df_enhancers.to_csv(
        data_path
        + f"/results/csv/distal_reg_paper/EG_features/{cell_line}/AllFeatures/{cell_line}_AllFeatures_NAfilled_withPreds.tsv",
        sep="\t",
        index=False,
    )
...
df_enhancers["GraphReg_LR_thresholded.Score"] = (df_enhancers["GraphReg.Score"] > 0.2247).astype(
    np.int32
)  # Threshold corresponding to recall = 0.7
df_enhancers_final = df_enhancers[
    [
        "chr",
        "start",
        "end",
...
df_enhancers_final = df_enhancers_final.rename(
    columns={"GraphReg.Score": "GraphReg_LR.Score", "distanceToTSS": "DistanceToTSS"}
)
df_enhancers_final["CellType"] = cell_line
df_enhancers_final["DistanceToTSS"] = df_enhancers_final["DistanceToTSS"].astype(np.int64)

if save_to_csv:
    df_enhancers_final.to_csv(
        data_path
        + f"/results/csv/distal_reg_paper/EG_features/{cell_line}/AllFeatures/SubmittedToENCODE/GraphRegLR_Predictions_{cell_line}.tsv",
        sep="\t",
        index=False,
    )
