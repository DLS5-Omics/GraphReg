...
if dataset == "fulco":
    ...
    print(f"Number of E-G pais in Fulco dataset: {len(df_crispr)}")
    print(f"Number of genes in Fulco dataset: {len(gene_list)}")
    ...
    df_crispr = df_crispr[df_crispr["ValidConnection"] == "TRUE"].reset_index(drop=True)
    gene_list = np.unique(df_crispr["measuredGeneSymbol"].values)
    print(f"Number of E-G pais in Gasperini dataset: {len(df_crispr)}")
    print(f"Number of genes in Gasperini dataset: {len(gene_list)}")
...
for fdr in ["1"]:  # ['001', '01', '1', '5', '9']:
    for saliency_method in ["saliency"]:
        for L in [2, 5, 10, 20, 40, 50]:
            ...
            df_crispr[f"Score_fdr_{fdr}_{saliency_method}_L_{L}"] = 0.0
            for i in range(len(df_crispr)):
                ...
                scores_gene = np.sqrt(
                    (np.exp(df_preds_gene["DNase"]) - 1)
                    * np.abs(df_preds_gene["Grad_DNase"])
                    * (np.exp(df_preds_gene["H3K27ac"]) - 1)
                    * np.abs(df_preds_gene["Grad_H3K27ac"])
                )
                ...
                if cnt == 1:
                    df_crispr.loc[i, "DistanceToTSS"] = np.abs(
                        enhancer_middle - df_preds_gene["TargetGeneTSS"].values[0]
                    )
                ...
                df_crispr.loc[i, f"Score_fdr_{fdr}_{saliency_method}_L_{L}"] = final_score

df_crispr.to_csv(
    data_path
    + "/results/csv/distal_reg_paper/"
    + dataset
    + "/EG_preds_only_enhancers_"
    + cell_line
    + "_"
    + genome
    + "_"
...
for fdr in ["1"]:
    for saliency_method in ["saliency"]:
        for L in [2, 5, 10, 20, 40, 50]:
            ...
            Y_pred = df_crispr[f"Score_fdr_{fdr}_{saliency_method}_L_{L}"].values
            ...
            plt.plot(
                ...
                precision,
                linewidth=3,
                label=f"fdr 0.{fdr} | {saliency_method} | L={L} | auPR={aupr:6.4f}",
            )

plt.title(
    "Fulco | all = {} | positives = {}".format(len(df_crispr), np.sum(df_crispr["Regulated"] == True)),
    fontsize=40,
)
...
plt.savefig(
    data_path + f"/results/csv/distal_reg_paper/PR_curve_{dataset}_500K-end.pdf",
    bbox_inches="tight",
)
