...
for fdr in ["001", "01", "1", "5", "9"]:
    for saliency_method in ["saliency"]:
        print(f"FDR: {fdr} | {saliency_method}")
        ...
        if dataset == "fulco":
            df_crispr = pd.read_csv(
                data_path + "/data/csv/CRISPR_benchmarking_data/EPCrisprBenchmark_Fulco2019_K562_GRCh38.tsv",
                delimiter="\t",
            )
            ...
            df_crispr = pd.read_csv(
                data_path
                + "/data/csv/CRISPR_benchmarking_data/EPCrisprBenchmark_Gasperini2019_0.13gStd_0.8pwrAt15effect_GRCh38.tsv",
                delimiter="\t",
            )
            df_crispr = df_crispr[df_crispr["ValidConnection"] == "TRUE"].reset_index(drop=True)
        elif dataset == "combined":
            df_crispr = pd.read_csv(
                data_path + "/data/csv/CRISPR_benchmarking_data/EPCrisprBenchmark_ensemble_data_GRCh38.tsv",
                delimiter="\t",
            )
        ...
        for i in range(len(df_crispr)):
            ...
            df_preds_gene = pd.read_csv(
                data_path
                + "/results/csv/distal_reg_paper/gradients_per_gene/"
                + cell_line
                + "/grads_"
                + genome
                + "_FDR_"
            ...
            if len(df_preds_gene) > 0:
                df_enhancer_middle = df_preds_gene[
                    (df_preds_gene["start"] <= enhancer_middle) & (df_preds_gene["end"] > enhancer_middle)
                ]
                df_tss_middle = df_preds_gene[
                    (df_preds_gene["start"] <= df_preds_gene["TargetGeneTSS"].values[0])
                    & (df_preds_gene["end"] > df_preds_gene["TargetGeneTSS"].values[0])
                ]
                ...
                if len(df_enhancer_middle) > 0 and len(df_tss_middle) > 0:
                    ...
                    for L in [2, 4, 8, 16, 32]:
                        ...
                        df_crispr.loc[i, f"H3K4me3_e_max_L_{L}"] = np.max(
                            2 ** (df_preds_around_enhancer["H3K4me3"].values) - 1
                        )
                        df_crispr.loc[i, f"H3K27ac_e_max_L_{L}"] = np.max(
                            2 ** (df_preds_around_enhancer["H3K27ac"].values) - 1
                        )
                        df_crispr.loc[i, f"DNase_e_max_L_{L}"] = np.max(
                            2 ** (df_preds_around_enhancer["DNase"].values) - 1
                        )
                        df_crispr.loc[i, f"H3K4me3_e_grad_max_L_{L}"] = np.max(
                            df_preds_around_enhancer["Grad_H3K4me3"].values
                        )
                        df_crispr.loc[i, f"H3K27ac_e_grad_max_L_{L}"] = np.max(
                            df_preds_around_enhancer["Grad_H3K27ac"].values
                        )
                        df_crispr.loc[i, f"DNase_e_grad_max_L_{L}"] = np.max(
                            df_preds_around_enhancer["Grad_DNase"].values
                        )
                        df_crispr.loc[i, f"H3K4me3_e_grad_min_L_{L}"] = np.min(
                            df_preds_around_enhancer["Grad_H3K4me3"].values
                        )
                        df_crispr.loc[i, f"H3K27ac_e_grad_min_L_{L}"] = np.min(
                            df_preds_around_enhancer["Grad_H3K27ac"].values
                        )
                        df_crispr.loc[i, f"DNase_e_grad_min_L_{L}"] = np.min(
                            df_preds_around_enhancer["Grad_DNase"].values
                        )

                        df_crispr.loc[i, f"H3K4me3_p_max_L_{L}"] = np.max(
                            2 ** (df_preds_around_tss["H3K4me3"].values) - 1
                        )
                        df_crispr.loc[i, f"H3K27ac_p_max_L_{L}"] = np.max(
                            2 ** (df_preds_around_tss["H3K27ac"].values) - 1
                        )
                        df_crispr.loc[i, f"DNase_p_max_L_{L}"] = np.max(2 ** (df_preds_around_tss["DNase"].values) - 1)
                        df_crispr.loc[i, f"H3K4me3_p_grad_max_L_{L}"] = np.max(
                            df_preds_around_tss["Grad_H3K4me3"].values
                        )
                        df_crispr.loc[i, f"H3K27ac_p_grad_max_L_{L}"] = np.max(
                            df_preds_around_tss["Grad_H3K27ac"].values
                        )
                        df_crispr.loc[i, f"DNase_p_grad_max_L_{L}"] = np.max(df_preds_around_tss["Grad_DNase"].values)
                        df_crispr.loc[i, f"H3K4me3_p_grad_min_L_{L}"] = np.min(
                            df_preds_around_tss["Grad_H3K4me3"].values
                        )
                        df_crispr.loc[i, f"H3K27ac_p_grad_min_L_{L}"] = np.min(
                            df_preds_around_tss["Grad_H3K27ac"].values
                        )
                        df_crispr.loc[i, f"DNase_p_grad_min_L_{L}"] = np.min(df_preds_around_tss["Grad_DNase"].values)

        df_crispr.to_csv(
            data_path
            + "/results/csv/distal_reg_paper/"
            + dataset
            + "/features_"
            + cell_line
            + "_"
...
for fdr in ["001", "01", "1", "5", "9"]:
    for saliency_method in ["saliency"]:
        if dataset in ["gasperini", "combined"]:
            ...
            df_crispr = df_crispr[
                (~df_crispr["DNase_e_max_L_2"].isna()) & (~df_crispr["Regulated"].isna())
            ].reset_index(drop=True)
        for L in [2, 4, 8, 16, 32]:
            print(f"FDR: {fdr} | {saliency_method} | L = {L}")
            if dataset == "fulco":
                df_crispr = pd.read_csv(
                    data_path
                    + "/results/csv/distal_reg_paper/"
            ...
            if dataset in ["gasperini", "combined"]:
                X[:, 0] = df_crispr[f"H3K4me3_e_max_L_{L}"].values
                X[:, 1] = df_crispr[f"H3K27ac_e_max_L_{L}"].values
                X[:, 2] = df_crispr[f"DNase_e_max_L_{L}"].values
                X[:, 3] = df_crispr[f"H3K4me3_e_grad_max_L_{L}"].values
                X[:, 4] = df_crispr[f"H3K27ac_e_grad_max_L_{L}"].values
                X[:, 5] = df_crispr[f"DNase_e_grad_max_L_{L}"].values
                X[:, 6] = df_crispr[f"H3K4me3_e_grad_min_L_{L}"].values
                X[:, 7] = df_crispr[f"H3K27ac_e_grad_min_L_{L}"].values
                X[:, 8] = df_crispr[f"DNase_e_grad_min_L_{L}"].values
                X[:, 9] = df_crispr[f"H3K4me3_p_max_L_{L}"].values
                X[:, 10] = df_crispr[f"H3K27ac_p_max_L_{L}"].values
                X[:, 11] = df_crispr[f"DNase_p_max_L_{L}"].values
                X[:, 12] = df_crispr[f"H3K4me3_p_grad_max_L_{L}"].values
                X[:, 13] = df_crispr[f"H3K27ac_p_grad_max_L_{L}"].values
                X[:, 14] = df_crispr[f"DNase_p_grad_max_L_{L}"].values
                X[:, 15] = df_crispr[f"H3K4me3_p_grad_min_L_{L}"].values
                X[:, 16] = df_crispr[f"H3K27ac_p_grad_min_L_{L}"].values
                X[:, 17] = df_crispr[f"DNase_p_grad_min_L_{L}"].values
            ...
            if save_model:
                with open(
                    data_path
                    + "/results/csv/distal_reg_paper/{}/models/model_{}_FDR_{}_L_{}.pkl".format(
                        dataset, dataset, fdr, L
                    ),
                    "wb",
                ) as f:
                    pickle.dump(clf, f)
            ...
            plt.plot(
                ...
                precision,
                linewidth=3,
                label="FDR=0.{} | L={} | auPR={:6.4f} | Recall={:4.2f} | Precision={:4.2f} | Threshold={:4.2f}".format(
                    fdr,
                    L,
                    aupr,
                    recall_at_70_pct,
                    precision_at_70_pct_recall,
                    threshod_in_70_pct_recall,
                ),
            )
...
"""
plt.title(
    "{} | all = {} | positives = {}".format(dataset, len(df_crispr), np.sum(df_crispr["Regulated"] == True)),
    fontsize=40,
)
...
plt.grid()
plt.savefig(
    data_path + f"/results/csv/distal_reg_paper/figs/PR_curve_LRScores_{dataset}_all.pdf",
    bbox_inches="tight",
)
...
plt.title(
    "{} | all = {} | positives = {}".format(dataset, len(df_crispr_sub), np.sum(df_crispr_sub["Regulated"] == True)),
    fontsize=40,
)
plt.legend(bbox_to_anchor=(1.01, 1), borderaxespad=0, fontsize=40)
plt.xlabel("Recall", fontsize=40)
...
plt.grid()
plt.savefig(
    data_path + f"/results/csv/distal_reg_paper/figs/PR_curve_LRScores_{n_folds}FoldCV_{dataset}_best_all.pdf",
    bbox_inches="tight",
)
                ...
                if len(idx_test) > 0:
                    ...
                    if dataset in ["gasperini", "combined"]:
                        ...
                        if save_model:
                            with open(
                                data_path
                                + "/results/csv/distal_reg_paper/{}/models/model_{}_FDR_{}_L_{}_test_{}.pkl".format(
                                    dataset, dataset, fdr, L, chr
                                ),
                                "wb",
                            ) as f:
                                pickle.dump(clf, f)
                        ...
                        if load_combined_model:
                            with open(
                                data_path
                                + "/results/csv/distal_reg_paper/combined/models/model_combined_FDR_{}_L_{}_test_{}.pkl".format(
                                    fdr, L, chr
                                ),
                                "rb",
                            ) as f:
...
# save to tsv
df_crispr_sub.to_csv(
    data_path
    + "/results/csv/distal_reg_paper/"
    + dataset
    + "/EG_preds_on_holdout_chrs_"
    + cell_line
    + "_"
    + genome
    + "_"
...
plt.plot(
    ...
    precision,
    linewidth=3,
    label="Ensemble | auPR={:6.4f} | Recall={:4.2f} | Precision={:4.2f} | Threshold={:4.2f}".format(
        aupr, recall_at_70_pct, precision_at_70_pct_recall, threshod_in_70_pct_recall
    ),
)
plt.title(
    "{} | all = {} | positives = {}".format(dataset, len(df_crispr), np.sum(df_crispr["Regulated"] == True)),
...
plt.grid()
plt.savefig(
    data_path + f"/results/csv/distal_reg_paper/figs/PR_curve_LRScores_{n_folds}FoldCV_{dataset}_ensemble_all.pdf",
    bbox_inches="tight",
)
...
for fdr in ["001", "01", "1", "5", "9"]:
    for saliency_method in ["saliency"]:
        ...
        for L in [2, 4, 8, 16, 32]:
            ...
            # load model
            with open(
                data_path + f"/results/csv/distal_reg_paper/{model}/models/model_{model}_FDR_{fdr}_L_{L}.pkl",
                "rb",
            ) as f:
                clf = pickle.load(f)
...
plt.figure(figsize=(20, 20))
plt.plot(recall, precision, linewidth=3, label=f"Ensemble | auPR={aupr:6.4f}")
plt.title(
    "{} to {} | all = {} | positives = {}".format(
        model, dataset, len(df_crispr), np.sum(df_crispr["Regulated"] == True)
    ),
    fontsize=40,
)
plt.legend(bbox_to_anchor=(1.01, 1), borderaxespad=0, fontsize=40)
...
df_crispr_1 = pd.read_csv(
    data_path + "/results/csv/distal_reg_paper/" + dataset1 + "/EG_preds_K562_hg38_gasperini_FDR_001_saliency_L_32.tsv",
    delimiter="\t",
)
df_crispr_2 = pd.read_csv(
    data_path + "/results/csv/distal_reg_paper/" + dataset2 + "/EG_preds_K562_hg38_fulco_FDR_5_saliency_L_8.tsv",
    delimiter="\t",
)

df_crispr = (
    df_crispr_1[["Regulated", "DistanceToTSS", "Score"]]
    .append(df_crispr_2[["Regulated", "DistanceToTSS", "Score"]])
    .reset_index(drop=True)
)
...
plt.plot(
    ...
    precision,
    linewidth=3,
    label="auPR={:6.4f} | \n Recall={:4.2f} | Precision={:4.2f} | Threshold={:4.2f}".format(
        aupr, recall_at_70_pct, precision_at_70_pct_recall, threshod_in_70_pct_recall
    ),
)
plt.title(
    "Merged datasets (best preds) | all = {} | positives = {}".format(
        len(df_crispr), np.sum(df_crispr["Regulated"] == True)
    ),
    fontsize=40,
)
plt.legend(bbox_to_anchor=(1.01, 1), borderaxespad=0, fontsize=40)
...
plt.grid()
plt.savefig(
    data_path + "/results/csv/distal_reg_paper/figs/PR_curve_LRScores_merged_datasets_best_CVpreds_all.pdf",
    bbox_inches="tight",
)
