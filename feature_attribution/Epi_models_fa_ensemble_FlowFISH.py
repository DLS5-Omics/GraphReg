                    ...
                    if tf.reduce_sum(tf.gather(tss_idx, idx)) > 0:
                        if (
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= 1:
                                ...
                                average_precision_abc = np.append(
                                    average_precision_abc,
                            ...
                            if saliency_method == "deepshap":
                                if load_fa == False:
                                    explain_output_idx = (
                                        np.floor((gene_tss_list[i] - pos[0]) / 5000).astype("int64").reshape([1, 1])
                                    )
                                    ...
                                    for j in range(1, 1 + 10):
                                        ...
                                        (
                                            shap_values_cnn_,
                                            indexes_cnn,
                                        ) = explainer_cnn.shap_values(
                                            X_epi.numpy(),
                                            ranked_outputs=explain_output_idx,
                                            output_rank_order="custom",
                                            nsamples=200,
                                        )
                                        print(
                                            type(shap_values_cnn_[0]),
                                            shap_values_cnn_[0].shape,
                                        )
                                ...
                                if load_fa == False:
                                    ...
                                    for j in range(1, 1 + 10):
                                        ...
                                        with tf.GradientTape(persistent=True) as tape:
                                            ...
                                            preds = model_cnn(inp)
                                            target_cnn = (
                                                preds[:, explain_output_idx_cnn - 1]
                                                + preds[:, explain_output_idx_cnn]
                                                + preds[:, explain_output_idx_cnn + 1]
                                            )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= 1:
                                ...
                                average_precision_cnn = np.append(
                                    average_precision_cnn,
                            ...
                            if saliency_method == "deepshap":
                                if load_fa == False:
                                    ...
                                    for j in range(1, 1 + 10):
                                        ...
                                        model_gat_1 = tf.keras.Model(
                                            inputs=model_gat.inputs,
                                            outputs=model_gat.outputs[0],
                                        )

                                        explainer_gat = shap.GradientExplainer(
                                            model_gat_1,
                                            [x_background, adj.numpy()],
                                            batch_size=1,
                                        )
                                        (
                                            shap_values_gat_,
                                            indexes_gat,
                                        ) = explainer_gat.shap_values(
                                            [X_epi.numpy(), adj.numpy()],
                                            nsamples=200,
                                            ranked_outputs=explain_output_idx,
                                            output_rank_order="custom",
                                        )
                            ...
                            for s_idx, e_idx in zip(start_idx, end_idx):
                                cnt = cnt + 1
                                mid_idx = int((s_idx + e_idx) / 2)
                                gat_fa_gene = np.append(
                                    gat_fa_gene,
                                    np.max(scores_gat[mid_idx - L : mid_idx + L + 1]) / np.max(scores_gat),
                                )
                            ...
                            if len(Y_true) >= 10 and len(idx_sig) >= 1:
                                ...
                                sp_gat = np.append(
                                    sp_gat,
                                    spearmanr(gat_fa_gene, expr_change_percent_flowfish)[0],
                                )
                                ...
                                average_precision_gat = np.append(
                                    average_precision_gat,
                                    average_precision_score(Y_true, Y_pred_gat),
                                )
                                auprc_gat = np.append(auprc_gat, auc(recall_gat, precision_gat))
                                print(
                                    "Epi-GraphReg AP score: ",
                                    average_precision_score(Y_true, Y_pred_gat),
                                )
                                print(
                                    "Epi-GraphReg AUPRC score: ",
                                    auc(recall_gat, precision_gat),
                                )
                            ...
                            if write_bw == True:
                                bw_flowfish = pyBigWig.open(
                                    data_path
                                    + "/results/bigwig/feature_attribution/Epi-models/flowfish_"
                                    + cell_line
                                    + "_"
                                    + gene_names_list[i]
                                    + ".bw",
                                ...
                                bw_abc = pyBigWig.open(
                                    data_path
                                    + "/results/bigwig/feature_attribution/Epi-models/abc_"
                                    + cell_line
                                    + "_"
                                    + gene_names_list[i]
                                    + ".bw",
                                ...
                                bw_GraphReg = pyBigWig.open(
                                    data_path
                                    + "/results/bigwig/feature_attribution/Epi-models/Epi-GraphReg_"
                                    + saliency_method
                                    + "_"
                                    + cell_line
                                    + "_"
                                ...
                                bw_CNN = pyBigWig.open(
                                    data_path
                                    + "/results/bigwig/feature_attribution/Epi-models/Epi-CNN_"
                                    + saliency_method
                                    + "_"
                                    + cell_line
                                    + "_"
                                ...
                                bw_flowfish.addEntries(
                                    chroms,
                                    starts,
                                    ends=ends,
                                    values=expr_change_percent_vec,
                                )
                                ...
                                with open(
                                    data_path
                                    + "/results/bigwig/feature_attribution/Epi-models/candidates_"
                                    + cell_line
                                    + "_"
                                    + gene_names_list[i]
                                    + ".bed",
                                    ...
                                    for s_idx, e_idx in zip(start_flowfish, end_flowfish):
                                        ...
                                        line = (
                                            ...
                                            + str(e_idx)
                                            + "\t"
                                            + str(significant_flowfish_bool[cnt])
                                        )
                                        bed_file.write(line)
                                        bed_file.write("\n")
...
# plt.ylim((0,14))
plt.title(
    "SP (significant decrease): " + str(spearmanr(abc_all[idx], expr_change_percent_all[idx])[0].astype(np.float16)),
    fontsize=20,
)
...
# plt.ylim((0,14))
plt.title(
    "SP (significant decrease): " + str(spearmanr(gat_fa_all[idx], expr_change_percent_all[idx])[0].astype(np.float16)),
    fontsize=20,
)
# plt.plot(np.linspace(0,1,100),np.linspace(0,1,100),color='red')
plt.tick_params(axis="x", labelsize=25)
...
# plt.ylim((0,14))
plt.title(
    "SP (significant decrease): " + str(spearmanr(cnn_fa_all[idx], expr_change_percent_all[idx])[0].astype(np.float16)),
    fontsize=20,
)
# plt.plot(np.linspace(0,1,100),np.linspace(0,1,100),color='red')
plt.tick_params(axis="x", labelsize=25)
...
plt.plot(
    ...
    precision_gat,
    color="red",
    label="Epi-GraphReg: AUC = " + str(auprc_gat_all.astype(np.float16)),
)
plt.plot(
    recall_cnn,
    precision_cnn,
    color="green",
    label="Epi-CNN: AUC = " + str(auprc_cnn_all.astype(np.float16)),
)
plt.plot(
    recall_abc,
    precision_abc,
...
print(
    "Epi-GraphReg SP for significants: ",
    spearmanr(gat_fa_all[idx], expr_change_percent_all[idx])[0],
)
print(
    "Epi-CNN SP for significants: ",
    spearmanr(cnn_fa_all[idx], expr_change_percent_all[idx])[0],
)
