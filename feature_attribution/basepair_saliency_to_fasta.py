...
df = df[
    (
        ...
        & (df["n_tss"] == 1)
        & (df["n_contact"] >= 5)
        & (np.abs(df["delta_nll"]) > 300)
        & (df["nll_seq_graphreg"] < 300)
    )
...
GraphReg_motifs_fasta = open(
    data_path + "/results/fimo/Seq-GraphReg_distal_motifs_" + cell_line[0] + ".fasta",
    "w",
)
                    ...
                    if pos[-1] < 1e15 and gene_tss_list[i] >= pos[0] + 2000000 and gene_tss_list[i] < pos[0] + 4000000:
                        ...
                        if saliency_method == "saliency":
                            if load_fa == False:
                                ...
                                np.save(
                                    data_path
                                    + "/results/numpy/feature_attribution/Seq-CNN_BP_grad_by_inp"
                                    + "_"
                                    + cell_line
                                    + "_"
                                    + gene_names_list[i]
                                ...
                                np.save(
                                    data_path
                                    + "/results/numpy/feature_attribution/Seq-CNN_BP_grad"
                                    + "_"
                                    + cell_line
                                    + "_"
                                    + gene_names_list[i]
                        ...
                        if saliency_method == "saliency":
                            if load_fa == False:
                                ...
                                np.save(
                                    data_path
                                    + "/results/numpy/feature_attribution/Seq-GraphReg_BP_grad_by_inp"
                                    + "_"
                                    + cell_line
                                    + "_"
                                    + gene_names_list[i]
                                ...
                                np.save(
                                    data_path
                                    + "/results/numpy/feature_attribution/Seq-GraphReg_BP_grad"
                                    + "_"
                                    + cell_line
                                    + "_"
                                    + gene_names_list[i]
                        ...
                        idx_cnn = np.where(
                            np.logical_and(
                                np.abs(pos[0] + start_pos + 10 - gene_tss_list[i]) > distal_thr,
                                np.abs(box_cnn_mean) > 0,
                            )
                        )[0]
                        idx_gat = np.where(
                            np.logical_and(
                                np.abs(pos[0] + start_pos + 10 - gene_tss_list[i]) > distal_thr,
                                np.abs(box_gat_mean) > 0,
                            )
                        )[0]
                        ...
                        num_top_scores = (
                            25 * gene_contact_list[i]
                        )  # top 5% : (5000/10)*N/20 = 25N (N: number of contacts)
                        ...
                        print(
                            "box_cnn_mean_idx significant: ",
                            box_cnn_mean_idx[idx_sig_cnn],
                        )
                        print(
                            "box_cnn_mean_idx non-significant: ",
                            box_cnn_mean_idx[idx_non_sig_cnn],
                        )
                        print(
                            "box_gat_mean_idx significant: ",
                            box_gat_mean_idx[idx_sig_gat],
                        )
                        print(
                            "box_gat_mean_idx non-significant: ",
                            box_gat_mean_idx[idx_non_sig_gat],
                        )
                        ...
                        for j in idx_sig_gat:
                            line = (
                                ...
                                + gene_names_list[i]
                                + "_"
                                + str(box_gat_mean_idx[j])
                                + "_"
                                + chrm
                                + ":"
                                + str(pos[0] + start_pos_idx_gat[j])
                                + "-"
                                + str(pos[0] + start_pos_idx_gat[j] + 20)
                            )
                            GraphReg_motifs_fasta.write(line)
                            GraphReg_motifs_fasta.write("\n")
                            seq_dna = fasta_open.fetch(
                                chrm,
                                pos[0] + start_pos_idx_gat[j],
                                pos[0] + start_pos_idx_gat[j] + 20,
                            )
                        ...
                        for j in idx_sig_cnn:
                            line = (
                                ...
                                + gene_names_list[i]
                                + "_"
                                + str(box_cnn_mean_idx[j])
                                + "_"
                                + chrm
                                + ":"
                                + str(pos[0] + start_pos_idx_cnn[j])
                                + "-"
                                + str(pos[0] + start_pos_idx_cnn[j] + 20)
                            )
                            CNN_motifs_fasta.write(line)
                            CNN_motifs_fasta.write("\n")
                            seq_dna = fasta_open.fetch(
                                chrm,
                                pos[0] + start_pos_idx_cnn[j],
                                pos[0] + start_pos_idx_cnn[j] + 20,
                            )
                        ...
                        if write_bw == True:
                            bw_GraphReg = pyBigWig.open(
                                data_path
                                + "/results/bigwig/feature_attribution/Seq-models_basepair/Seq-GraphReg_BP_"
                                + saliency_method
                                + "_"
                                + cell_line
                                + "_"
                            ...
                            bw_CNN = pyBigWig.open(
                                data_path
                                + "/results/bigwig/feature_attribution/Seq-models_basepair/Seq-CNN_BP_"
                                + saliency_method
                                + "_"
                                + cell_line
                                + "_"
