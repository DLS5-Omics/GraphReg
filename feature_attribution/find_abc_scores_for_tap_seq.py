...
for i in range(len(TAP_seq_enhancer_df)):
    ...
    ABC_K562_df_sub = ABC_K562_df_sub[
        ((tap_start >= ABC_K562_df_sub["start"]) & (tap_start <= ABC_K562_df_sub["end"]))
        | ((tap_end >= ABC_K562_df_sub["start"]) & (tap_end <= ABC_K562_df_sub["end"]))
        | ((tap_start <= ABC_K562_df_sub["start"]) & (tap_end >= ABC_K562_df_sub["end"]))
    ]
