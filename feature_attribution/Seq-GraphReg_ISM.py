...
def calculate_loss(
    ...
    for i, chr in enumerate(chr_list):
        ...
        tss_pos = np.load(
            data_path + "/data/tss/" + organism + "/" + genome + "/tss_pos_chr" + str(chr) + ".npy",
            allow_pickle=True,
        )
        gene_names_all = np.load(
            data_path + "/data/tss/" + organism + "/" + genome + "/tss_gene_chr" + str(chr) + ".npy",
            allow_pickle=True,
        )
            ...
            if data_exist:
                if tf.reduce_sum(tf.gather(tss_idx, idx)) > 0:
                    if pos[-1] < 1e15 and gene_tss_list[i] >= pos[0] + 2000000 and gene_tss_list[i] < pos[0] + 4000000:
                        ...
                        sp.sparse.save_npz(
                            data_path
                            + "/results/numpy/feature_attribution/Seq-GraphReg_BP_ISM_"
                            + condition
                            + "_"
                            + cell_line
                            + "_"
                            + gene_names_list[i]
                            + ".npz",
                            sparse_matrix_gat_ism,
                        )

                        sparse_matrix_cnn_ism = sp.sparse.csr_matrix(ISM_scores_cnn.flatten())
                        sp.sparse.save_npz(
                            data_path
                            + "/results/numpy/feature_attribution/Seq-CNN_BP_ISM_"
                            + condition
                            + "_"
                            + cell_line
                            + "_"
                        ...
                        if write_bw == True:
                            bw_GraphReg = pyBigWig.open(
                                data_path
                                + "/results/bigwig/feature_attribution/Seq-models_basepair/Seq-GraphReg_BP_ISM_"
                                + condition
                                + "_"
                                + cell_line
                                + "_"
                            ...
                            bw_CNN = pyBigWig.open(
                                data_path
                                + "/results/bigwig/feature_attribution/Seq-models_basepair/Seq-CNN_BP_ISM_"
                                + condition
                                + "_"
                                + cell_line
                                + "_"
                            ...
                            bw_GraphReg.addEntries(
                                ...
                                starts,
                                ends=ends,
                                values=ISM_scores_gat_sum.ravel()[idx_pos],
                            )
                            bw_CNN.addEntries(
                                chroms,
                                starts,
                                ends=ends,
                                values=ISM_scores_cnn_sum.ravel()[idx_pos],
                            )
...
df_JUND = TF_positions_df[
    (TF_positions_df["TF"] == "JUND") & (TF_positions_df["chr"] == "chr19") & (TF_positions_df["start"] < 4000000)
]
...
##### plot ISM heatmaps #####
sparse_matrix_gat_ism = sp.sparse.load_npz(
    data_path
    + "/results/numpy/feature_attribution/Seq-GraphReg_BP_ISM_"
    + condition
    + "_"
...
plt.tight_layout()
fig.savefig(
    "../figs/Seq-models/final/scatterplot/heatmap_ISM_TCF3_enhancer_chr19_2945347-2945447.pdf",
    bbox_inches="tight",
)
