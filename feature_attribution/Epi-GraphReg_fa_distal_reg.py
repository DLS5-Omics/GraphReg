    ...
    for num, cell_line in enumerate(cell_lines):
        for i, chrm in enumerate(chr_list):
            ...
            enhancer_gene_score.write(
                "chr\tstart\tend\tname\tclass\tTargetGene\tTargetGeneTSS\tCellType\tH3K4me3\tH3K27ac\tDNase\tGrad_H3K4me3\tGrad_H3K27ac\tGrad_DNase\tDistanceToTSS"
            )
                                    ...
                                    for j in range(1, 1 + 11):
                                        ...
                                        if gene_tss_list[i] % 5000 >= 4800:
                                            ...
                                            (
                                                ...
                                                [X_epi.numpy(), adj.numpy()],
                                                nsamples=200,
                                                ranked_outputs=explain_output_idx + 1,
                                                output_rank_order="custom",
                                            )
...
if crispr_dataset == "fulco":
    ...
    print(f"Number of E-G pais in Fulco dataset: {len(crispr_data)}")
    print(f"Number of genes in Fulco dataset: {len(gene_names_list)}")
    ...
    crispr_data = crispr_data[crispr_data["ValidConnection"] == "TRUE"].reset_index(drop=True)
    gene_names_list = np.unique(crispr_data["measuredGeneSymbol"].values)
    print(f"Number of E-G pais in Gasperini dataset: {len(crispr_data)}")
    print(f"Number of genes in Gasperini dataset: {len(gene_names_list)}")
    ...
    gene_names_list = np.unique(crispr_data["measuredGeneSymbol"].values)
    print(f"Number of E-G pais in combined dataset: {len(crispr_data)}")
    print(f"Number of genes in combined dataset: {len(gene_names_list)}")
elif crispr_dataset == "none":
    tss_dataframe_sub = tss_dataframe[
        tss_dataframe["chr"].isin(["chr" + str(i) for i in range(1, 23)] + ["chrX"])
...
if crispr_dataset != "none":
    ...
    if crispr_dataset == "fulco":
        df.loc[3, "tss"] = tss_dataframe[tss_dataframe["gene"] == "C19orf43"]["tss_1"].values[
            0
        ]  # Replace TSS of C19orf43
    elif crispr_dataset == "combined":
        df.loc[1181, "tss"] = tss_dataframe[tss_dataframe["gene"] == "C19orf43"]["tss_1"].values[
            0
        ]  # Replace TSS of C19orf43
