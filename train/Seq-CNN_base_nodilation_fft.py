...
def main():
                        ...
                        if tf.reduce_sum(tf.gather(tss_idx, idx)) > 0:
                            for jj in range(20, 40, b_size):
                                ...
                                rho_h3k4me3_all = np.append(
                                    rho_h3k4me3_all,
                                    np.corrcoef(
                                        np.log2(Y_h3k4me3_batch.numpy().ravel() + 1) + e1,
                                        np.log2(Y_hat_h3k4me3.numpy().ravel() + 1) + e2,
                                    )[0, 1],
                                )
                                rho_h3k27ac_all = np.append(
                                    rho_h3k27ac_all,
                                    np.corrcoef(
                                        np.log2(Y_h3k27ac_batch.numpy().ravel() + 1) + e1,
                                        np.log2(Y_hat_h3k27ac.numpy().ravel() + 1) + e2,
                                    )[0, 1],
                                )
                                rho_dnase_all = np.append(
                                    rho_dnase_all,
                                    np.corrcoef(
                                        np.log2(Y_dnase_batch.numpy().ravel() + 1) + e1,
                                        np.log2(Y_hat_dnase.numpy().ravel() + 1) + e2,
                                    )[0, 1],
                                )
    ...
    if re_load:
        ...
        model_cnn_base._name = "Seq-CNN_base_nodilution_fft"
    ...
    for epoch in range(1, n_epochs + 1):
                            ...
                            for jj in range(20, 40, b_size):
                                ...
                                with tf.GradientTape() as t2:
                                    with tf.GradientTape(watch_accessed_variables=False) as t1:
                                        ...
                                        L_c = (1 / 3) * (
                                            poisson_loss(Y_h3k4me3_batch, Y_hat_h3k4me3)
                                            + poisson_loss(Y_h3k27ac_batch, Y_hat_h3k27ac)
                                            + poisson_loss(Y_dnase_batch, Y_hat_dnase)
                                        )

                                    grad_input = t1.gradient(H, seq_batch)
                                    g = grad_input * seq_batch
                                    g = K.sum(g, axis=-1)
                                    g = tf.reshape(g, [-1])
                                    g = tf.abs(g)
                                ...
                                grads = t2.gradient(loss, model_cnn_base.trainable_variables)
        ...
        if epoch % 1 == 0:
            (
                ...
                valid_rho_h3k27ac,
                valid_rho_dnase,
            ) = calculate_loss(model_cnn_base, valid_chr_list, cell_lines, b_size)
        ...
        if valid_loss < best_loss:
            ...
            (
                ...
                test_rho_h3k27ac,
                test_rho_dnase,
            ) = calculate_loss(model_cnn_base, test_chr_list, cell_lines, b_size)
            print(
                "test epoch: ",
                epoch,
                "total loss: ",
